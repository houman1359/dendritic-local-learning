...
MANIFEST_PATH="${5:-${REPO_ROOT}/drafts/dendritic-local-learning/neurips/configs/publication_sweeps_${OUTPUT_TAG}.txt}"
...
"${REPO_ROOT}/drafts/dendritic-local-learning/neurips/scripts/regenerate_publication_bundle.sh" \
  "${MANIFEST_PATH}" \
  "${OUTPUT_TAG}" \
  "${ANALYSIS_DIR}" \
  "${FIGURE_DIR}"
