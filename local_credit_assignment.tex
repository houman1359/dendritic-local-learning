...
\end{theorem}
\begin{proof}
Apply the multivariate chain rule on the directed acyclic computation graph defined by the tree; use Proposition~\ref{eq:grad_V}. Each path contributes a product of edge sensitivities. Summing over parents produces \eqref{eq:tree_recursion}; unrolling yields \eqref{eq:path_sum}.
\end{proof}
...
For minibatch size $B \geq 2$, $\rho_n$ is estimated directly from \eqref{eq:rho} with exponential moving average (EMA). For $B = 1$ (online learning), use Welford's numerically stable algorithm \cite{welford1962note}:
\begin{align}
...
\begin{definition}[Unique-Variance Information Factor]
\label{def:phi_fixed}
...
\begin{theorem}[Path-Sum Approximation Bias]
\label{thm:path_bias}
Let $\pi_n$ be defined by \eqref{eq:path_factor}. The path factor approximates the sum over all paths in \eqref{eq:path_sum}. If (i) $R_i^{\mathrm{tot}}$ and outgoing $g^{\mathrm{den}}$ at each depth have coefficient of variation $\le \kappa<1$, and (ii) the number of paths from $n$ to $0$ is $P_n$, then for a chain (single path):
\begin{equation}
\pi_n = \prod_{i=0}^{n-1} R_i^{\mathrm{tot}} g_i^{\mathrm{den}}.
\end{equation}
For general trees, $\pi_n$ approximates the dominant path contribution when per-depth conductances are relatively homogeneous. Modulating the error by $\pi_n$ yields:
\begin{equation}
...
\end{theorem}
\begin{proof}[Sketch]
For chains, the recursion exactly recovers the single-path product. For trees, $\pi_n$ uses layer-averaged conductances, providing an approximation to the sum-over-paths when path conductances are similar. Apply concentration bounds on products per depth.
\end{proof}
...
\midrule
Conductance scaling $R_n^{\mathrm{tot}}$ & Input resistance & Lemma~\ref{lem:convex}: $0 < R_n \le 1$ \\
Driving force $(E_j - V_n)$ & Synaptic current & Prop.~\ref{eq:grad_gsyn}: Local sensitivity \\
Shunting inhibition & Divisive normalization & Sec.~\ref{sec:shunting}: $\partial V/\partial g_{\text{inh}} \propto -V$ \\
Path factor $\pi_n$ & Cable attenuation & Thm.~\ref{thm:path_bias}: Depth decay \\
Morphology factor $\rho_n$ & Layer correlation & Eq.~\eqref{eq:rho}: Task relevance \\
Information factor $\phi_n$ & Conditional SNR & Eq.~\eqref{eq:phi}: $1/(1-R^2)$ \\
