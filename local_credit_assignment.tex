...
Additionally, shunting self-normalization produces log-normal weight distributions whose dispersion matches connectomic data (MICrONS, $\Delta\sigma < 0.08$).
...
\item \textbf{Biological weight distributions.}
Shunting self-normalization produces log-normal weight distributions matching connectomic and electrophysiological data, with dispersion controlled by the E/I synapse ratio.
\end{enumerate}
...
\subsection{Finding 6: Biological Weight Distributions}
...
Such multiplicative dynamics predict log-normal weight distributions in trained networks, consistent with the log-normal synaptic weight distributions observed in cortex \cite{song2005lognormal,buzsaki2014logdynamic}.
...
\paragraph{BP+Shunting matches electrophysiology; Local+Shunting matches connectomics.}
BP+Shunting produces $\sigma = 0.94$ on MNIST, matching Song et al.\ almost exactly ($\Delta = 0.008$).
Local+Shunting produces $\sigma = 1.13$--$1.22$, matching MICrONS E$\to$E synapse sizes ($\sigma = 1.14$; $\Delta \leq 0.08$).
By contrast, Local+Additive produces $\sigma = 1.43$--$1.69$, substantially broader than any biological reference.
...
Our framework predicts that (1)~stronger perisomatic inhibition yields more precise synaptic plasticity; (2)~GABA\textsubscript{A} blockade selectively impairs multi-layer credit tasks; (3)~the dendritic--somatic voltage correlation ($\rho_n$) increases during learning; (4)~learned excitatory weight distributions follow log-normal statistics with dispersion matching connectomic data (Finding~6).
