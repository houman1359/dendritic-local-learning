...
\subsection{Empirical Program and Main Claim}

We evaluate the proposed local credit assignment (LocalCA) rules using a program designed to separate: (i) model capacity, (ii) local-rule competence, and (iii) mechanistic regime dependence.

\textbf{Main claim.} In compartmental dendritic networks, \emph{dendritic tree interactions and shunting inhibition create a regime in which strictly local updates become effective and interpretable}. In our current sweep set, the strongest seed-robust signal is:
\begin{enumerate}
\item \textbf{Decoder locality is sufficient:} local decoder updates match backprop decoder updates, while disabling decoder updates collapses performance (Fig.~\ref{fig:decoder_locality}).
\item \textbf{Shunting is the key regime:} shunting dendritic cores outperform additive controls under the same local rule, with the gap widening under stronger inhibition/noise (Figs.~\ref{fig:shunting_regime}, \ref{fig:claimA_shunting_heatmap}).
\item \textbf{Broadcast/path interactions are conditional:} path propagation reliably changes representation-level information metrics and sometimes improves accuracy depending on broadcast mode (Figs.~\ref{fig:source_interaction}, \ref{fig:phase_information_panel}).
\end{enumerate}
...
\subsection{Hypotheses and Falsifiers}

\paragraph{H1: Broadcast mode matters, but is regime-dependent.}
Changing \texttt{error\_broadcast\_mode} should produce reproducible shifts in learning dynamics and performance; the best broadcast mode may depend on task regime (inhibition/noise, depth, decoder update mode).  
\textbf{Falsifier:} no significant change across seeds or effects that fail to reproduce under matched conditions.

\paragraph{H2: Path propagation primarily shapes representations.}
Enabling \texttt{use\_path\_propagation} should produce consistent shifts in information metrics (MI/CMI proxies) and can improve accuracy in specific regimes, but is not expected to be uniformly accuracy-improving.  
\textbf{Falsifier:} no measurable representation shifts and no interaction with broadcast or morphology.

\paragraph{H3: Decoder learning can remain local.}
Under competent LocalCA settings, \texttt{decoder\_update\_mode=local} should match \texttt{backprop}, while \texttt{none} should degrade.  
\textbf{Falsifier:} local decoder materially underperforms backprop on the same models and data.

\paragraph{H4: The effect is regime-dependent and shunting-linked.}
Gains from LocalCA should be strongest in shunting/strong-inhibition and noise-stress regimes and weaker in additive controls.  
\textbf{Falsifier:} gains are equally large in additive controls or vanish under shunting.

\subsection{Results Snapshot (Completed Sweeps, Feb.~2026)}

\paragraph{Phase 1 ceilings (standard training).}
On the Phase-1 capacity calibration sweep, standard backprop achieves high accuracy on MNIST (best point MLP: 0.978 test; best dendritic shunting: 0.965 test) and context gating (best dendritic shunting: 0.864 test). On CIFAR-10 with flattened inputs, standard baselines reach \(\approx 0.49\) test accuracy (best point MLP). These ceilings provide the reference regime for Phase-2/3 local-rule comparisons.

\paragraph{Phase 2 competence (local\_ca).}
In a Phase-2b gap-closing pilot sweep (160 runs; 5 seeds), we find a large and reproducible dependence on broadcast mode and HSIC strength: \texttt{per\_soma} broadcast substantially outperforms \texttt{scalar} on both MNIST and context gating, and moderate HSIC weights (0.01--0.1) improve context-gating accuracy while overly large weights degrade performance (Fig.~\ref{fig:phase2b_hsic_weight}). Under the Phase-1 shunting capacity regime (layer sizes \([128]\), branch factors \([3,3]\), IE \(=20\)), LocalCA reaches \(0.914\pm0.003\) test accuracy on MNIST and \(0.799\pm0.018\) on context gating (with \(0.803\pm0.006\) achievable at HSIC weight 0.01), compared to standard-training ceilings of 0.965 and 0.864 respectively.

\paragraph{Phase 3 mechanism (regime dependence and information).}
The shunting regime advantage is largest in noise-stress settings (e.g., \texttt{noise\_resilience} shows \(\approx 0.19\) test-accuracy advantage for shunting over additive at higher inhibition under per-soma broadcast), while representation-level metrics show clear broadcast/path interactions (Fig.~\ref{fig:phase_information_panel}).
...
\caption{\textbf{Phase 2b gap closing: HSIC strength and broadcast mode.} For context gating, moderate HSIC weights improve LocalCA performance under \texttt{per\_soma} broadcast, while large weights degrade. For MNIST, HSIC does not improve over the best \(\lambda_{\mathrm{HSIC}}=0\) setting. Error bars are across seeds.}
...
\caption{\textbf{Shunting advantage across inhibition strength (Phase 3 Claim A).} Heatmap of the shunting minus additive test-accuracy difference over inhibitory synapse count (\texttt{IE}) and broadcast modes, highlighting the inhibition/noise regimes where shunting-linked local credit is most beneficial.}
...
\caption{\textbf{Information panel (Phase 3).} Information metrics and accuracy jointly reveal that dendritic mechanisms can systematically reshape representations even when end-task accuracy is only weakly affected, motivating mechanistic analyses beyond headline accuracy.}
...
\bottomrule
\end{tabular}
\caption{\textbf{Robust mechanistic headline results (small-network sandbox).} Values are grouped over robust multi-seed claim sweeps (\texttt{claim2--claim4}) and are intended to isolate mechanisms (decoder locality, shunting regime dependence, broadcast/path interaction), not to define the final performance ceiling.}
...
\begin{enumerate}
\item \textbf{F1:} Model schematic and local rule decomposition (3F/4F/5F + morphology-aware factors).
\item \textbf{F2:} Phase 1 capacity ceilings across cores/datasets (standard backprop).
\item \textbf{F3:} LocalCA competence vs the Phase-1 ceiling (selected datasets and architectures).
\item \textbf{F4:} Decoder-locality ablation (\texttt{local} vs.\ \texttt{backprop} vs.\ \texttt{none}).
\item \textbf{F5:} Shunting regime dependence (shunting vs additive across inhibition and noise stress).
\item \textbf{F6:} Broadcast-path interaction and information panel (accuracy vs MI/CMI, plus ablations).
\end{enumerate}
