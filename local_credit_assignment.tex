...
Additionally, the self-normalizing dynamics of shunting produce log-normal weight distributions whose dispersion matches connectomic data (MICrONS, $\Delta\sigma < 0.08$), providing a structural signature of biologically plausible learning.
...
Such multiplicative dynamics predict log-normal weight distributions in trained networks, matching a well-documented feature of biological synapses \cite{song2005lognormal,buzsaki2014logdynamic}.
...
Our framework predicts that (1)~stronger perisomatic inhibition yields more precise synaptic plasticity; (2)~GABA\textsubscript{A} blockade selectively impairs multi-layer credit tasks; (3)~the dendritic--somatic voltage correlation ($\rho_n$) increases during learning; (4)~learned excitatory weight distributions follow log-normal statistics with dispersion matching connectomic data (Finding~6), providing a testable structural signature of the conductance-based learning rule.
...
This is analogous to a learning rate that self-adjusts via the total conductance: strong synapses contribute more to $g_n^{\mathrm{tot}}$, reducing their own future update magnitude---a natural form of weight-dependent regularization.
...
Additive integration lacks this self-normalizing mechanism, producing broader and less biologically realistic distributions.
