#!/usr/bin/env python3
"""Audit and analyze the frozen CIFAR-10 shunting feedback ladder.
...
``analysis/CIFAR10_SHUNTING_FEEDBACK_LADDER_CONFIRMATORY_20260922.md``.
"""
...
EXPECTED_SEEDS = tuple(range(22000, 22020))
...
SELECTED_SCREEN_CONFIG = (
    ...
    "journal_cifar10_bp_recipe_init_screen_20260828101020/results/config_39/config.json"
)

CONDITIONS = {
    "cifar10_shunting_strict_scalar_confirmatory": {
        "label": "strict scalar",
        "strategy": "local_ca",
        "broadcast": "scalar",
    },
    "cifar10_shunting_neuron_specific_confirmatory": {
        "label": "neuron specific",
        "strategy": "local_ca",
        "broadcast": "per_soma_shared",
    },
    "cifar10_shunting_exact_path_confirmatory": {
        "label": "exact path",
        "strategy": "local_ca",
        "broadcast": "path_transport",
    },
    "cifar10_shunting_matched_bp_confirmatory": {
...
EXPECTED_RESOLVED_FIELDS = {
    ...
    "model.core.type": "dendritic_shunting",
    ...
    "model.core.morphology.use_shunting": True,
    ...
    "training.main.common.epochs": 400,
    ...
    "training.main.common.patience": 50,
    ...
    "training.main.common.weight_decay_rate": 0.0,
    ...
    "training.main.common.param_groups.split_params": True,
...
def selected_screen_evidence() -> dict:
    ...
    selected = nested_get(summary, "decision.selection.shunting.selected")
    ...
    if selected != "shunting_empirical_local_matched":
...
def active_optimizer_semantics(resolved: dict) -> dict:
    """Audit the selected shunting recipe's active parameter-group rates."""
    fields = {key: value for key, value in EXPECTED_RESOLVED_FIELDS.items()
              if key.startswith("training.main.common.param_groups.")
              or key.startswith("training.main.optimizer.")
              or key == "training.main.common.weight_decay_rate"}
    rates = {group: nested_get(resolved, f"training.main.common.param_groups.{group}_lr")
             for group in ("topk", "blocklinear", "reactivation", "decoder")}
    return {
        "valid": not field_errors(resolved, fields),
        "optimizer": nested_get(resolved, "training.main.optimizer.name"),
        "split_params": nested_get(resolved, "training.main.common.param_groups.split_params"),
        "active_group_learning_rates": rates,
        "sparse_active_weight_maintenance_rate": nested_get(resolved, "training.main.common.weight_decay_rate"),
        ...
        rows.append(
            {
                ...
                "active_group_learning_rates": json.dumps(optimizer["active_group_learning_rates"], sort_keys=True),
