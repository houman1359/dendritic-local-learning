#!/usr/bin/env python3
"""Main Figure 5: local inhibitory selection and learned branch tuning.

A–B show the shared-input identity-output teacher and learning signals;
C–D retain the frozen aligned/opposed learning curves; E retains assignment,
placement and rate controls; F–G show all twenty selected-state branch curves.
The paired target contrast and earlier cancellation diagnostic are rendered
in Supplementary S22 by checkpoint_figure.py using the retained helpers below.

No training or outcome selection occurs here. The portable completion audit
checks the original protocol and numerical summaries before rendering. The
current panel-source map, display CSV and focused provenance describe the
published layout; older layout decisions remain historical records.
"""
import hashlib
import json
import math
...
from review_completion import population_figure as population_figures  # noqa: E402
...
def interaction(ax, t):
    ...
    ax.annotate('zero difference', xy=(1.5, 0.0), xytext=(0.0, -1.4),
...
def display_rows(tables, printed):
    ...
    for rate in RATES:
        ...
        for _, r in sel.iterrows():
            rows.append(dict(panel='E', record='endpoint', task=r.task,
                             budget=r.budget, rate=r.rate, rule=r.rule,
                             seed=r.seed, best_step=r.best_step,
                             test_nmse=r.test_nmse,
                             fixed_endpoint_test_nmse=r.fixed_endpoint_test_nmse,
        ...
        for rule in f_rules:
            m = cond(tables, 'opposed_strong', rule, rate=rate)
            rows.append(dict(panel='E', record='condition mean',
                             task='opposed_strong', budget=PRIMARY['budget'],
                             rate=rate, rule=rule, n=int(m.n), mean=m['mean'],
                             ci_low=m.ci_low, ci_high=m.ci_high,
                             mean_fixed_endpoint=m.mean_fixed_endpoint))
...
def figure_provenance(cfg, printed, problems, audit):
    ...
    inputs += list(sorted((J / 'source_data/curated_publication').glob('inhibitory_rescue_*')))
    ...
    code += [J / 'scripts/review_completion/population_figure.py',
             J / 'scripts/review_completion/promoted_panels.py']
    inputs += [J / 'source_data/checkpoint_computation/branch_tuning.csv']
...
def main():
    ...
    focused_provenance.publish(
        ...
        builders=[Path(__file__), Path(__file__).with_name('report.py'),
                  ...
                  J / 'scripts/native_schematics.py',
                  J / 'scripts/review_completion/population_figure.py',
                  J / 'scripts/review_completion/promoted_panels.py',
