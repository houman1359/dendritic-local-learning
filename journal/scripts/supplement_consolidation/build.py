...
remain immutable inputs. New lettering is embedded DejaVu Sans Bold.
"""
...
from specification import FIGURES, REMOVED_SHARED_REGIONS, SHARED_LEGENDS, WHOLE_CROPS, PANEL_BOUNDS, PANEL_REDACTIONS, PANEL_PATCHES, PANEL_TEXT, NATIVE_LEGENDS, EXPLICIT_NUMERICAL_SOURCES
...
def layout(items):
 """Prefer two readable panels per row; never compress a panel below 90%."""
 best=None
 for counts in partitions(len(items)):
  ...
  for count in counts:
   ...
   rowgutter=6. if count==3 else GUTTER
   scale=min(1.05,(W-2*MARGIN-rowgutter*(count-1))/width)
   height=max(q['height']*scale for q in sub)+15
   rows.append((sub,scale,height));scales.append(scale)
  height=sum(x[2] for x in rows)+GUTTER*(len(rows)-1)+2*MARGIN
  # Respect adequate type; slight extra height is preferable to shrinking.
  penalty=sum(max(0,.96-v)**2*2000000 for v in scales)+max(0,height-585)**2/15+height*.05+sum(4 for n in counts if n==3)
  if best is None or penalty<best[0]:best=(penalty,rows,height)
...
def clean_caption(s):
 ...
 ('Both cohorts are independent of Supplementary Fig.~\\ref{fig:supp_morphology_credit}.','Both cohorts are independent of Supplementary Fig.~\\ref{fig:si_oracle_profile_credit}.')]:s=s.replace(old,new)
 return s
...
def main():
 ...
 assets=[];mapping={k:[] for k in REG};captions={}
 for number,(ident,module,groups,title,caption) in enumerate(FIGURES,1):
  ...
  if ident in WHOLE_CROPS:
   ...
   rows,height=layout(items);legend_specs=SHARED_LEGENDS.get(ident,[]);legend_h=sum((r[3]-r[1])+10 for _,r in legend_specs)+(22 if ident in NATIVE_LEGENDS else 0);output=fitz.open();p=output.new_page(width=W,height=height+legend_h)
   p.insert_font(fontname='PanelSans',fontfile=font)
   y=MARGIN;nextletter=0
   for sub,scale,rowh in rows:
    rowgutter=6. if len(sub)==3 else GUTTER
    ...
    for q in sub:
     ...
     clip=fitz.Rect(q['bbox']);target=fitz.Rect(x,y+15,x+clip.width*scale,y+15+clip.height*scale)
     p.show_pdf_page(target,source,0,clip=clip)
     for tt in PANEL_TEXT.get((key,oldletter),[]):
      origin=[target.x0+(tt['origin'][0]-clip.x0)*scale,target.y0+(tt['origin'][1]-clip.y0)*scale];p.insert_font(fontname='ReflowAxis',fontfile=font_manager.findfont('DejaVu Sans'));p.insert_text(origin,tt['text'],fontname='ReflowAxis',fontsize=tt['fontsize']*scale,rotate=tt.get('rotate',0));ancillary.append(dict(tt,target_source_panel=key+oldletter,target_origin=origin))
     newletter=chr(65+nextletter);nextletter+=1
     p.insert_text((target.x0+1,y+10.4),newletter,fontname='PanelSans',fontsize=10.5,color=(.1,.12,.13))
   ...
   for key,r in legend_specs:
    ...
    clip=fitz.Rect(r);target=fitz.Rect((W-clip.width)/2,y-7,(W+clip.width)/2,y-7+clip.height);p.show_pdf_page(target,source,0,clip=clip);ancillary.append({'role':'shared legend','source':key,'source_asset':REG[key]['path'],'source_sha256':REG[key]['sha256'],'bbox':list(clip),'target_rect':list(target)});y+=clip.height+10
   if ident in NATIVE_LEGENDS:
    p.insert_font(fontname='LegendSans',fontfile=font_manager.findfont('DejaVu Sans'));x=105
    for entry in NATIVE_LEGENDS[ident]:
     yy=y+1;p.draw_line((x,yy-3),(x+11,yy-3),color=entry['color'],width=1.4);p.insert_text((x+15,yy),entry['label'],fontname='LegendSans',fontsize=7.8);ancillary.append(dict(entry,role='native shared legend',source='S14',origin=[x,yy]));x+=110
  if args.only is None or args.only==ident:
   output.set_metadata({'title':title or re.sub(r'\\textbf\{([^}]+)\}.*',r'\1',originals[groups[0][0]],flags=re.S),'author':'Safaai, Richards and Sabatini','creator':'supplement_consolidation/build.py; native vector-panel reflow','keywords':json.dumps({'schema':'native-vector-reflow/1','id':ident,'panels':panelmap},separators=(',',':'))})
  ...
  captions[ident]=cap
  assets.append({'figure':f'S{number}','id':ident,'module':module,'path':str(dest.relative_to(J)),'label':label,'panels':panelmap,'ancillary_elements':ancillary,'whole_source_sheet':whole,'width_pt':output[0].rect.width,'height_pt':output[0].rect.height,'sha256':sha(dest) if dest.exists() else None})
  print(number,ident,'height',round(output[0].rect.height,1),'min_scale',round(min(p['scale'] for p in panelmap),3))
 for key in mapping:
  if not mapping[key]:
   fallback={'S1':('note:derivation','Exact-gradient derivation; original numerical panels remain in the source archive.'),'S6':('note:conflict_ancestry','Static quadratic interference remains in the derivation; the original numerical illustration is archived.'),'S16':('note:interior_optimum','Designed and retrospective optimum comparisons remain numerically in the operator-theory note; the full diagnostic is archived.'),'S17':('tab:animal_credit','External signed neuron-level result remains tabulated.'),'S26':('tab:physical_reproducibility','Same-seed rerun outcomes and outliers remain in the physical-depth reproducibility table.')}.get(key)
