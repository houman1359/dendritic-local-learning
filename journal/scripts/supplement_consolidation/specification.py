...
('mechanistic_chain','si_01_exact',[('S1','*')],
 ...
 r'''MNIST panels \textbf{A--C} use two-stage $[4,4]$ directed trees; the separate noise-resilience panels \textbf{D,E} use two-stage $[3,3]$ additive trees. \textbf{A}, Conductance-stage path-gain field in shunting and normalized-additive models at five inhibitory synapses per branch; the inset shows coefficients of variation (CV; standard deviation divided by mean), with means and whiskers of one sample standard deviation across five paired seeds; small points show the seeds. The schematic is drawn on the shared balanced tree of main Fig.~1B rather than the $[4,4]$ geometry: one root-to-leaf route carries the serial path gains \(\alpha_1\), \(\alpha_2\), \(\alpha_3\), distal sites are tinted by their illustrative path gain (darker, larger) in each model's colour, and \(\delta_0\) is the somatic error. This loading quantity omits branch derivatives and is distinct from the trained exact-error profiles in Supplementary Fig.~S45D,E. \textbf{B}, Stage-resolved cosine of matched-width (MW) scalar-fallback versus exact MNIST errors; the width-matched somatic stage is excluded because its cosine is one by construction. \textbf{C}, MNIST post-training inhibition interventions: zero, sample shuffle, per-branch batch mean and a uniform matched mean. The dashed rule marks ten-class chance. These also change forward computation and do not isolate backward shunting. \textbf{D,E}, Feedback fidelity and learning on the additive noise task; solid and dashed lines denote matched-width scalar-fallback and exact transport. Bars show standard deviations across checkpoints in \textbf{B} and retained runs in \textbf{C}; curves and shaded bands in \textbf{D,E} show means $\pm$ one standard deviation across five retained runs per condition; white points identify checkpoints in \textbf{B} and runs in \textbf{C}. Noise-task shunting aggregates with unresolved execution lineage are excluded and retained in Source Data only. The retained additive aggregates do not resolve their normalization setting. The state-matched focal analysis in main Fig.~9 provides the conductance-specific transport comparison.'''),
...
# ---------------------------------------------------------------- si_04 ----
('scalar_tree_capacity','si_04_interactions_boolean',[('N12','*')],
...
('fixed_profile_budget','si_04_interactions_boolean',[('N16','*')],
...
('morphology_estimation','si_10_selection_statistics',[('N36','*')],
 ...
 r'''\textbf{A}, Flow chart of the sealed calibration protocol (Section~\ref{note:finite_calibration_morphology}): green, estimator branch; orange, two-sweep pilot branch; merging in the sealed reset-and-train step. \textbf{B}, Pooled fixed-baseline-minus-estimated (left, grey) and pilot-minus-estimated (right, orange) NMSE regret: small dots, 80 per-task paired differences; open squares, family means; short rule, pooled mean; whisker, Bonferroni-adjusted 97.5\% paired interval; solid rule, zero; the 0.01-NMSE margin lies below both lower confidence bounds (0.083, 0.027). \textbf{C}, Pooled menu regret (excess clean-test NMSE above the best trained candidate) against calibration labels: solid curves, filled markers, noise SD 0.5; dashed, open markers, no noise; bands, 95\% intervals; the fixed / estimated-rank baseline is one grey curve for both noise levels, and the estimated-cut noise contrast lies within the seed interval (1,024 labels: 0.022 [0.010, 0.035] noiseless, 0.033 [0.015, 0.056] at SD 0.5). \textbf{D}, Family-specific primary regret (green, estimated interactions; grey, fixed/estimated-rank baseline; orange, pilot): small dots, 20 per-task regrets; short rule, mean; whisker, 95\% interval; the quartic estimated-cut interval (0.0027 [0.0014, 0.0046]) is narrower than the rule. \textbf{E}, Secondary adaptive trees (green; ALS fits) against the best trained candidate (grey) and the true-target tree (purple), clean-test NMSE, marks as in \textbf{D}. \textbf{F}, Selection time on one CPU (eighty primary tasks; axis from zero): dots, per-task times; short rule, median (0.046 s, estimation plus scoring, green; 0.066 s, twelve two-sweep pilots, orange; 1.4-fold); whisker, interquartile range. \textbf{G}, Fresh Adam cohort: estimated (green), development-fixed (grey) and target-informed (purple) trees under exact credit (filled) and root broadcast (open), noisy-test NMSE: small dots, 20 per-seed endpoints; whisker, 95\% interval; dashed rule, label-noise floor 0.0225. \textbf{H}, Pooled paired noisy-test differences in neutral ink: small dots, 20 seed differences; open squares, family means (matching, quartic, nested, random, left to right); short rule, pooled mean; whisker, Bonferroni-adjusted 97.5\% interval for the two primary contrasts and pointwise 95\% for estimated-minus-target-informed; solid rule, zero. Target-informed construction is not a performance ceiling. Intervals in \textbf{C--E} and \textbf{G}: pointwise 95\%, whole-seed bootstrap, $n=20$.'''),
]
...
PANEL_REASONS = {
 'S17':'The six-animal inference is reported in Table S7 and the protocol in Section S9. The descriptive figure is retained in the source archive; it adds no within-tree evidence.',
 ...
 'S38':'Replaced by the native render N35 (scripts/build_supplementary_figure_finite_horizon_native.py), drawn from the same source tables and pasted whole as Supplementary Fig. S34; no crop of this frozen render is used.',
 'S39':'Replaced by the native render N35 (scripts/build_supplementary_figure_finite_horizon_native.py), drawn from the same source tables and pasted whole as Supplementary Fig. S34; no crop of this frozen render is used.',
 'S41':'Replaced by the native render N36 (scripts/build_supplementary_figure_morphology_estimation_native.py), drawn from the same source tables and pasted whole as Supplementary Fig. S35; no crop of this frozen render is used.',
...
PANEL_CONTENT = {
 ...
 ('S11','A'): dict(content='Shunt-minus-current-injection localization across fixed absolute doses at three membrane resistances and three background multipliers.',
                   reason='Replaced by the native render N29 (scripts/build_supplementary_figure_shunt_sensitivity_native.py), drawn from the same source tables and pasted whole as Supplementary Fig. S29; no crop of this render is used.',
...
for _letter,_content,_source in [
 ...
 PANEL_CONTENT[('S17',_letter)]=dict(content=_content,
  reason='Redundant external display omitted; the inferential results remain in Table S7 and the protocol in Section S9.',
...
PANEL_CONTENT[('S31','G')]['reason']='The distinct exact-path LocalCA comparison remains in Supplementary Figure S22C; main Figure 7C uses exact BP.'
...
PANEL_CONTENT[('M9','D')]['reason']='Prediction outcomes remain in Table S6 and are compared with linear baselines in Figure S32A; the repeated absolute-error display was removed.'
