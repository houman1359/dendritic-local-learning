...
\textbf{E}, Spatial share $C-C_0$ at $K=8$ for six dictionaries; dashed line, broadcast capture $C_0=0.203$, for scale, removed from every bar; cell means, no interval; $n=47$ cells.
...
\textbf{G}, Paired ancestry-minus-control advantages at $K=8$ on residual (filled) and total (open) capture scales (one unit equals 100 percentage points), with all 47 within-cell differences; means and 95\% cell-bootstrap intervals, four of the 188 differences clipped to the axis edge; Right: nonzero-entry fraction of a dense dictionary, mean rank and positive-cell count.
\textbf{H}, Residual capture $C_{\rm res}=(C-C_0)/(1-C_0)$ in three cohorts from two mice (initial and disjoint, one MICrONS mouse; Pinky, a second); means with 95\% cell-bootstrap intervals at $K=8$; dots, cells; $n=8$, 47 and 8.
