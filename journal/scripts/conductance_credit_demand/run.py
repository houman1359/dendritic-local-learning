...
OUT=HERE.parents[1]/'source_data/conductance_credit_demand'
...
def default_config():
 return dict(development_seeds=[101,102,103],fresh_seeds=list(range(1101,1121)),
  tasks=[dict(name='ungated_independent',conflict=0.,gate=0.),dict(name='gated_independent',conflict=0.,gate=10.),dict(name='gated_conflict',conflict=1.,gate=10.),dict(name='moderate_conflict',conflict=1.,gate=1.)],
  ...
  task_selection=dict(primary='gated_conflict',fallback_order=['moderate_conflict','gated_independent'],control='ungated_independent',teacher_rank_one_capture_max=.9,exact_adam_validation_nmse_max=.02,rule='First ordered task meeting teacher geometry and exact Adam validation criteria, never based on between-rule gap. All candidates retained; no passing candidate means no confirmatory spatial-credit claim.'),
  ...
  main_contrast='Gated minus ungated difference in calibrated-broadcast minus exact clean test NMSE, paired by fresh teacher/initialization/data/minibatches. Report unit control, SGD, common-rate, endpoints and all seeds.',
  forward_scope='Same directed seven-compartment E/I equation and all 16 trainable log conductances as existing conductance reference; identity reactivation, no arbitrary multiplication or quartic representability assumption.',
...
def freeze():
 ...
 record=dict(protocol_sha256=sha(p),source_sha256={str(s.relative_to(HERE.parents[1])):sha(s) for s in [HERE/'run.py',HERE/'model.py',HERE/'test_model.py',model.REFERENCE]},python=platform.python_version(),numpy=np.__version__,pandas=pd.__version__)
...
def run_task(seed,task,phase,cfg,steps=None):
 ...
 variance=float(np.var(y));initrng=np.random.default_rng(np.random.SeedSequence([seed,45678]));initial=np.log(model.NOMINAL)+initrng.normal(0,.4,16);theta=np.tile(initial,(n,1))
 ...
 teacher=model.teacher(seed)[None];teacherprofile=model.forward(teacher,cx)['path'][:,:,:6].mean(axis=1);teachergeom=model.metrics(teacher,dx,dy,variance,teacherprofile)[0]
 metadata=dict(seed=seed,task=task,phase=phase,records=records,training_target_variance=variance,test_target_variance=float(np.var(ty)),clean_noise_floor=0.,teacher_log_conductances=teacher[0].tolist(),teacher_geometry=teachergeom,constructive_forward_nmse=float(np.mean((model.forward(teacher,tx)['output'][0]-ty)**2)/np.var(ty)),all_16_conductances_trainable=True,wall_seconds=time.perf_counter()-start,source='scripts/morphology_conductance/model.py',slurm_job_id=os.environ.get('SLURM_JOB_ID'))
