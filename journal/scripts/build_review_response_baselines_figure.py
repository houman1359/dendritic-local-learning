...
import numpy as np
import pandas as pd
from journal_style import (apply_neurips_style, COLORS, LW_ERR, LW_REF,
...
def main():
    ...
    a.axvline(1, color=COLORS["mute"], lw=.7, ls=":", zorder=1)
    ...
    a.tick_params(axis="y", labelsize=7, length=0)
    ...
    ac.axvline(0, color=COLORS["mute"], lw=.7, ls=":", zorder=1)
    ...
    ac.tick_params(axis="y", labelsize=7, length=0)
    ac.xaxis.label.set_size(7)
    style_axis(ac, grid="x")

    def sequence(ax, keys, x, color, marker="o"):
        values = summary.loc[keys]
        for _, row in targets[keys].iterrows():
            ax.plot(x, row.to_numpy(), color=color, alpha=.4, lw=.7, zorder=1)
        ...
        ax.errorbar(x, values.mean_nmse, yerr=np.vstack([
            ...
            fmt=marker+"-", color=color, ms=4, capsize=2, lw=1.25, zorder=3)
