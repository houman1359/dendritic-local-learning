...
def main():
    ...
    for credit,label,color,marker,ls in SPECS:
        ...
        original_means[credit]=means
        original_handles[credit]=ax.errorbar(part.depth,means,yerr=[means-100*part.ci95_low_test_accuracy,100*part.ci95_high_test_accuracy-means],
    ...
    original_overlap_max_pp=float(np.max(np.abs(original_means['soma_broadcast_matched']-original_means['local_shared'])))
    assert original_overlap_max_pp < .064, 'The original LocalCA-recipe accuracy curves no longer nearly overlap'
    original_legend=[original_handles['full_bp'],original_handles['soma_broadcast_bp'],
        original_handles['local_path'],(original_handles['soma_broadcast_matched'],original_handles['local_shared'])]
    ax.legend(original_legend,['Exact; BP recipe','Broadcast; BP recipe','Exact path; LocalCA',
        'Broadcast / shared soma;\nLocalCA recipe (near overlap)'],handler_map={tuple:HandlerTuple(ndivide=None)},
        loc='upper left',ncol=2,frameon=False,fontsize=PT_LEGEND,columnspacing=1.7)
    ...
    for (arm,depth),part in curves.groupby(['arm','depth']):
        ...
        # Both trajectories remain drawn. Distinct styles and a joint legend
        # disclose the near overlap instead of implying a missing condition.
        if arm=='broadcast_autograd_localca_recipe':ls='--'
        if arm=='per_soma_shared':ls=':'
        line,=ax.plot(mean.index,mean,color=color,ls=ls,label=label)
        trajectory_lines[(arm,depth)]=line
    ...
    broadcast=curves[curves.arm.eq('broadcast_autograd_localca_recipe')].groupby('epoch').best_validation_loss.mean()
    shared=curves[curves.arm.eq('per_soma_shared')].groupby('epoch').best_validation_loss.mean()
    overlap_max=float(np.max(np.abs(broadcast-shared)))
    assert overlap_max < .001, 'The LocalCA-recipe trajectories no longer nearly overlap'
    handles=[trajectory_lines[('exact_autograd_bp_recipe',3)],
        trajectory_lines[('broadcast_autograd_bp_recipe',3)],
        trajectory_lines[('path_transport',3)],
        trajectory_lines[('exact_autograd_bp_recipe',1)],
        (trajectory_lines[('broadcast_autograd_localca_recipe',3)],trajectory_lines[('per_soma_shared',3)])]
    labels=['Exact; BP recipe','Broadcast; BP recipe','Exact path; LocalCA',
        'D1 exact; BP recipe','Broadcast / shared soma;\nLocalCA recipe (near overlap)']
    ax.legend(handles,labels,handler_map={tuple:HandlerTuple(ndivide=None)},loc='upper center',bbox_to_anchor=(.5,-.26),ncol=3,frameon=False,fontsize=PT_SMALL,columnspacing=1.5)
    ax=axes[2]
