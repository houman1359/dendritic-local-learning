...
to their historical implementations and execution records, and gives a recipe
for the fully resolved CPU package set. The compatibility constraints are not
a universal lock for historical scientific runs. The public NumPy ceiling,
NumPy 2 scientific execution and NumPy 1 CPU replay must remain separately
identified.
...
## Population rescue and bounded SGD check

`code/population_replay/` includes the exact allowlisted source snapshot used by
the population studies, including pre-existing production edits. Its original
protocol and source-hash inventory are retained; the nominal production HEAD
is not used as a substitute for the actual executed bytes. The verified launcher
replays original-bound Adam rescue, common-rate and nonlinear-separable controls,
or any of the 18 validation-only SGD development conditions. Its README specifies
new output directories and explicitly excluded short smoke runs. Raw checkpoints
remain in the separately retained experiment archives.
