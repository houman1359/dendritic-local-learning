...
\subsection*{Reference tables and source-data index}

\begin{table}[tbp]
\centering
\caption{Core notation. Symbols are defined at first use; this index distinguishes quantities that recur in different coordinate systems.}
\label{tab:notation}
\small
...
$g_i,x_i,g_ix_i$; $E_i^{\rm rev}$ & Maximal conductance, presynaptic activity, open conductance and reversal potential; E/I polarity does not require signed conductances. \\
$G_n^{\rm tot},R_n^{\rm tot}$ & Total open conductance and its reciprocal input resistance. \\
$\delta^V_n$, $\delta^a_n$; $q_n$ & Errors in voltage/activity coordinates; root-to-unit output derivative. In reciprocal cables, $\bm q$ denotes the current-source adjoint. \\
$\Phi$, $K$, $\bm c$ & Spatial-profile dictionary, column budget and delivered coefficients. Rank can be smaller than column count. \\
$\Gamma$ & Route-gain matrix in the delivery equation; the rectangular anatomical kernel of Eq.~\ref{eq:s_kernel} is separately defined. \\
$W$, $P_{\Phi,W}$; $\mathcal C_K$ & Positive coordinate weights, weighted projector and explicitly specified per-field or aggregate energy capture. \\
$\bm g$, $\bm d$, $\bm m$, $V$ & Exact gradient, routed-update estimator, its conditional mean and covariance. \\
$M$, $\Sigma$, $U$ & Fixed routing operator, estimator-noise covariance and one-step moment bound. \\
$H_{\rm c},D_{\rm r}$; $H_{\rm p},D_{\rm p}$ & Task-credit hierarchy/routed resolution, and latent task hierarchy/physical stage count. \\
...
Images & Original MNIST/Fashion/CIFAR: 15/10/20 seeds, 90/60/80 fits; random feedback 120 new fits; fresh dictionary 108 development and 190 fresh fits & \ref{note:credit_images}; twelve short dictionary checks excluded. \\
...
Physical composition & Ten seeds per cohort; H3 270, point/optimizer 200, dose 90, grouped H3/H2 220, H4 360, task families 360; same-seed repeat 430 and budget extension sixty & \ref{note:depth}; finite budgets and metric dependence retained. \\
