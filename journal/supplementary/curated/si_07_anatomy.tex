...
The eight-cell uncertainty analysis crosses mapping thresholds 2, 5 and $10\,\mu$m with direct-only or hybrid labels (proxy probability at least 0.8). Radius factors are $\exp(\sigma_r z-\sigma_r^2/2)$ for independent standard Gaussian $z$, with $\sigma_r=0,0.25,0.5$ and five paired draws at each nonzero dose. Every condition reselects up to four routes by the same leverage. Jaccard overlap, the number of routes shared by two selections divided by the number in their union, measures route changes; capture uses the fixed nominal field, sites and weights, isolating dictionary sensitivity rather than changing both sides of the comparison.
