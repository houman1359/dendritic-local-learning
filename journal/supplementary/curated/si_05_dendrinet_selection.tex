...
\end{equation}
At fixed weights, the prefactor is constant across examples, but it changes
as the coupling conductances learn. It is $\kappa_{ui}$ in the main-text
factorization. With identity
...
For the interaction target, a cue-only terminal signal omits another useful dependence. At fixed parameters, a terminal eligibility $e(z_1,c)$ depends on its own feature and context. Independence and symmetry give
\[
\mathbb E[e(z_1,c)\tanh z_1\tanh z_2\mid c]
=\mathbb E[e(z_1,c)\tanh z_1\mid c]\mathbb E[\tanh z_2]=0.
\]
The same argument applies with the feature indices exchanged and with a cue-only gain multiplying the eligibility. It concerns the target-interaction contribution to the expected terminal update; prediction-dependent terms and other parameter blocks need not vanish. The parent slope depends jointly on the two features and can break the factorization. This motivates the rescue but does not prove convergence.
...
\subsubsection*{Raw-gradient scale identity at archived states}
\label{sec:gradient_scale_identity}
At fixed parameters, the augmented terminal log-conductance gradient equals the exact gradient divided by the positive example-independent factor $\kappa_{ui}$ in main Eq.~8. This statement also holds after minibatch averaging. It does not equate optimizer histories or trajectories. Table~\ref{tab:population_coordinates} distinguishes terminal, proximal, somatic and readout differentiation and shows why proximal eligibility already includes its own output derivative.

A deterministic checkpoint-only check used all 120 validation-selected original-bound Adam states (six rules, twenty seeds), plus the twenty paired initial states. At each state, raw gradients before clipping were computed on the same 512 diagnostic examples, normalized by training-target variance. Across all 280 terminal parameter-block comparisons, the maximum relative Euclidean residual between the exact gradient and $\kappa$ times the augmented gradient was $1.08\times10^{-15}$ (maximum absolute component residual $1.05\times10^{-17}$, rounded upward). All scales were positive. Checkpoint and frozen-runtime hashes were verified; no training or new independent cohort was added. The pre-execution protocol, per-state results and input hashes accompany \path{curated_publication/scaled_gradient_identity.csv} and its provenance record.
