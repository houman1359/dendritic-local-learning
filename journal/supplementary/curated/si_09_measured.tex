...
For partner pair $i,j$, shared-path fraction is the soma-to-common-ancestor
path length divided by the shorter of their two soma-to-contact path lengths
(denominator floored at $10^{-9}\,\mu$m). The same value is used in the
observed-data statistic and its simulated counterpart. The covariance kernel
used to generate simulated responses below has a different, geometric-mean
normalization, chosen to guarantee positive semidefiniteness.
...
\subsection{External comparison of neuron-specific teaching signs}
...
two-coordinate energy. The descriptive neuron-level distributions in sheet ``5E'' remain in Source Data and do not increase the animal-level sample size. Table~\ref{tab:animal_credit} reports the animal-level estimates, intervals and sign tests.
