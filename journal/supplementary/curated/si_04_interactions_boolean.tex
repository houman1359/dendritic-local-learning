...
\paragraph{Interaction degree, derivative energy and relative noise.}
\label{sec:degree_relative_noise}
For independent symmetric binary inputs, write the multilinear target as
$f(\bm x)=\sum_S a_S\prod_{i\in S}x_i$. Orthogonality of the product basis gives
$\operatorname{Var}(f)=\sum_{S\ne\varnothing}a_S^2$ and
$\E\|\nabla_x f\|^2=\sum_{S\ne\varnothing}|S|a_S^2$.
If every nonconstant term has degree $d$, the latter is
$d\,\operatorname{Var}(f)$. Thus the primary pairwise and quartic families,
whose derivative second moments both have trace two, necessarily have clean
variances one and one-half. This difference is required by the spectrum match;
it is not an independent variance manipulation. Equal absolute label-noise
variance gives different NMSE floors. The present noisy comparison therefore
does not hold relative noise fixed.
