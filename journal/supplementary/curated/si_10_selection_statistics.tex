...
\section{Structure selection, statistics and reproducibility}
...
A post hoc audit separated local prediction from endpoint selection. Exact calibration one-step decrease satisfied the bound in all 12,800 conditions. Initialization utility correlated with actual first-update independent-test decrease at 0.984/0.974 in the two arms, despite the failed final selector. Algebraic, covariance, derivative and source/selection checks passed; 120 independent endpoints replayed within $5.55\times10^{-17}$. Complete matrices, policies, outcomes and audits are in \texttt{source\_data/prospective\_morphology\_selection/}.

\subsection{Finite, noisy calibration of interaction-based tree selection}
\label{note:finite_calibration_morphology}

The finite-calibration study uses the eight-input, 28-coefficient multi-affine model and twelve-candidate pool of Section~\ref{note:interaction_structure}. Four equally weighted families are sampled: four disjoint pairs, two complementary quartets, nested products of degrees 2/4/6/8, and a random-interaction control selecting four distinct degree-2--8 monomials. Input permutations, coefficient signs and magnitudes $\mathcal U[0.5,1.5]$ vary by task; coefficients are normalized to unit squared norm. Thus every target has zero mean and variance one on uniform Rademacher inputs, but spectra need not match across this mixture.
...
For each actual candidate cut, the selector removes the constant inside row of the estimated Walsh matrix and measures its squared singular-value tail beyond rank one. It minimizes the maximum normalized cut tail, breaking $10^{-10}$ ties by summed tails and candidate identifier. True-coefficient maximum tails are bounds; noisy estimates and summed-tail tie-breaking are scores. A secondary subset dynamic program searches beyond the fixed pool at the primary calibration condition.
...
Twenty new seed blocks supply eighty tasks. Every calibration dataset, score, pilot choice and adaptive tree was hash-sealed before final endpoints. Two worker failures preceded fitting; hashed node-decoding and module-dispatch repairs left sealed choices unchanged. Final fitting uses separate 1,024-example training data with noise SD 0.3, 32 ALS sweeps, two paired random restarts and output-preserving gauge normalization. Independent 512-example noisy validation chooses the restart. Twelve fixed candidates, one estimated adaptive tree and one true-coefficient adaptive reference give 2,240 fits; fixed candidate outcomes are reused across calibration conditions.

Primary evaluation is clean-target MSE on 4,096 independent inputs, normalized by population variance one. Noisy-test and complete 256-pattern errors are secondary. Repeated patterns across streams are allowed, with independent observation identities and noise. Regret is selected error minus the retrospective minimum over validation-selected fixed-pool candidates; adaptive regret can be negative and is retained. Failed/nonfinite fits receive predefined loss $10^6$, with warnings and counts retained.

Four within-seed family contrasts are averaged before 10,000 whole-seed bootstrap draws. At 256 queries/noise 0.5, the two primary comparisons against development-best fixed and the two-sweep pilot use Bonferroni-adjusted 97.5\% intervals plus a 0.01-NMSE improvement margin. Other budgets, families and adaptive construction are descriptive/secondary. The procedure tests fresh functions within a fixed mixture; its exponential Walsh dictionary and ALS fitter do not establish scalable biological morphology selection. Full calibration and cost outcomes remain in Source Data and Supplementary Fig.~\ref{fig:si_morphology_estimation}.
...
Six full-horizon implementation checks (seed 721000) are excluded. A pre-fit interface repair preserved scientific settings and sealed choices. Independent reconstruction reproduced all eighty trees and saved-weight endpoints exactly. All 480 trajectories, states, protocols, repairs and exclusions are in \path{source_data/morphology_calibration/end_to_end}. This is a finite-data construction/learning pipeline within the stated class, not an endpoint-optimal or biological morphology law.
...
regret and strong baselines; panel D shows the joint-arm cache sensitivity.
...
Source Data contain resolved configurations, unit-level outcomes, exclusions, frozen selections and source hashes. The software archive includes the numerical reference implementations, data joins and figure builders. Private paths are translated to article-relative paths without changing numerical settings; manifests authenticate original inputs and each portability edit. The source index below maps compact tables to the full endpoint records.
