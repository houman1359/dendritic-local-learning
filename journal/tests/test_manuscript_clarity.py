...
def test_population_panel_has_its_own_learning_rule_key():
    ...
    for label in ("Exact BP", "Within-neuron broadcast", "Relative-resistance gate"):
...
def test_tasks_are_defined_before_population_outcomes():
    ...
    result = source.index("The population experiment supported this prediction.")
    assert target < severity < gate < result
    assert "changed both the target interactions and proximal nonlinearity" in source
...
def test_supplement_explains_shared_path_and_nmse_normalizations():
    ...
    assert "main Fig.~6 fixes" in guide and "Fig.~S9 fixes sixteen terminals" in guide
