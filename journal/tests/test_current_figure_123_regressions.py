...
def test_control_heatmap_uses_disclosed_row_normalization():
    ...
    np.testing.assert_allclose(actual, np.abs(original) / np.abs(original).max(axis=1)[:, None])
...
def test_derangement_label_explicitly_wraps_last_branch():
    fig = plt.figure(figsize=(169.5 / 72, 124 / 72))
    ax = fig.add_axes([0, 0, 1, 1])
    try:
        conflict.backward_credit_schematic(ax)
        labels = {text.get_text() for text in ax.texts}
        assert " = δ 1[b ≡ b* + 1]" in labels
        assert "modulo B; B → 1" in labels
