...
SPEC = importlib.util.spec_from_file_location(
    "analyze_cifar10_shunting_feedback_ladder_confirmatory", SCRIPT
)
...
def test_resolved_field_audit_rejects_additive_operator():
    config = {"model": {"core": {"morphology": {"use_shunting": False}}}}
    errors = MODULE.field_errors(config, {"model.core.morphology.use_shunting": True})
    assert errors and "use_shunting" in errors[0]


def test_input_mode1_semantics_reject_explicit_inhibitory_population(tmp_path):
    metadata = {"strategy": "standard", "broadcast": None}
    config = {
        "experiment": {"seed": 22000},
        "model": {
            "core": {
...
def test_shunting_optimizer_requires_active_split_rates_and_no_maintenance():
    config = {"training": {"main": {
        "common": {"weight_decay_rate": 0.0, "param_groups": {
            "split_params": True, "lr": 0.001, "topk_lr": 0.001,
            "blocklinear_lr": 0.0001, "reactivation_lr": 0.0001, "decoder_lr": 0.001}},
        "optimizer": {"name": "adam", "lr": 0.001, "weight_decay": 0.0}}}}
    semantics = MODULE.active_optimizer_semantics(config)
    assert semantics["valid"]
    assert semantics["active_group_learning_rates"]["reactivation"] == 0.0001
    config["training"]["main"]["common"]["param_groups"]["reactivation_lr"] = 0.001
    assert not MODULE.active_optimizer_semantics(config)["valid"]
    config["training"]["main"]["common"]["param_groups"]["reactivation_lr"] = 0.0001
    config["training"]["main"]["common"]["weight_decay_rate"] = 0.01
    assert not MODULE.active_optimizer_semantics(config)["valid"]
...
def test_condition_name_accepts_native_sweep_variant():
    generated = {
        "_sweep_variant": "cifar10_shunting_exact_path_confirmatory"
    }
    name, metadata = MODULE.condition_from_config(generated)
    assert name == "cifar10_shunting_exact_path_confirmatory"
