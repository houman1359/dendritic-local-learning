...
The pattern extended beyond the primary readout. With fixed random soma-level feedback, neuron-specific signals improved over scalar feedback by about eleven points, while exact paths added at most 0.09 points (fifteen paired seeds; Fig.~\ref{fig:framework}E; Supplementary Fig.~S30). Fashion-MNIST also favored neuronal feedback. On flattened CIFAR-10, exact paths reduced accuracy relative to neuronal feedback by 0.86 points (0.55--1.17), while remaining equivalent to matched backpropagation within the predefined margin (Fig.~\ref{fig:framework}E; Supplementary Fig.~S4).

These benchmarks show that a spatially structured error field need not confer a learning advantage on finer delivery. Supplementary Figs.~S4 and S8 retain further depth and task-generator controls. A one-step smoothness bound explains how restricting noisy updates can improve expected progress, but its initialization utility lost to a fixed maximum-budget tree in all twenty seed averages of a 12,800-fit prospective test, in both selection arms. The bound therefore concerns local update quality rather than optimal morphology selection (Methods; Supplementary Sections~S3--S4). We next introduce task structure that makes particular spatial distinctions useful.
...
Four terminal compartments fed two proximal compartments and a soma, using the directed E/I conductance balance (Methods). Both subtrees received the same positive feature pairs, $\exp(z_j)$ and $\exp(-z_j)$, for two independent latent inputs $z_j$. A binary context increased inhibitory conductance in one proximal compartment. Teacher excitation and inhibition produced increasing feature responses in both subtrees for the aligned task, but increasing responses in one and decreasing responses in the other for the opposed task. Teacher and student shared the circuit, ensuring representability. All 24 student conductances were plastic; paired tasks and credit rules shared generic, independently perturbed initial weights and example streams.

We compared exact credit, uniform broadcast, a fixed profile calibrated at initialization, and three fixed spatial patterns. Two patterns addressed the terminal descendants of the respective proximal branches; the third addressed the proximal pair. Their example-dependent coefficients were least-squares projections of the exact field, making this an oracle test of the dictionary's delivery capacity. Local synaptic and coupling eligibilities were unchanged across rules.

With equal learning-rate tuning, 20 fresh paired seeds showed a large task-dependent gap (Fig.~\ref{fig:conductancecredit}C,D). At validation-selected states within 4,096 Adam updates, mean held-out NMSE on the opposed task was 0.000020 with exact credit and 0.664 with the calibrated profile, whereas both learned the aligned task accurately. The opposed-minus-aligned difference in the calibrated-minus-exact gap was 0.664 (95\% paired interval, 0.511--0.804). Continuing every condition to 16,384 updates retained a contrast of 0.628 (0.464--0.774), positive in all 20 seeds (Fig.~\ref{fig:conductancecredit}E). The three-pattern dictionary remained close to exact credit (Fig.~\ref{fig:conductancecredit}C,D). SGD and restarts with much wider conductance bounds gave the same qualitative result (Supplementary Fig.~S51). A six-rate, budget-matched development sweep retained large broadcast errors (Supplementary Fig.~S52).

The gradient estimates reveal competing demands. At the same broadcast-trained weights, conditional mean gradients from the two contexts cancelled more strongly under broadcast than under exact credit (Fig.~\ref{fig:conductancecredit}F). Individual-example credit gradients still had nonnegative alignment with the exact gradient. Context-dependent branch selection therefore improved learning in a conductance circuit by preserving distinctions lost when gradient estimates were averaged. This result concerns a binary-context task and three supplied spatial patterns; it does not establish a general relationship between path-field rank and required feedback resolution.
...
The seven-compartment model uses identity branch outputs in Eq.~\ref{eq:directedvoltage}. Two independent $z_j\sim\mathcal U[-2,2]$ generate positive inputs $\exp(\pm z_j)$, supplied to both subtrees. Every terminal has two excitatory and two inhibitory gains. Binary context inhibits one proximal compartment; teacher tuning is aligned or opposed across the two branches. All 24 positive conductances are parameterized by their logarithms. Teacher log gains receive independent Gaussian perturbations of standard deviation 0.15, and student initialization uses 0.4. Full gains, generators and exact eligibility derivatives are given in Supplementary Section~S4.

Four credit rules share 2,048 training, 1,024 validation and 4,096 test examples, 128-example minibatches and the same initialization. A separate 512-example unlabeled sample fixes the calibrated profile and the three dictionary columns. Projection coefficients use the current exact field. Development comprised 288 fits across four retained regimes, two optimizers and three rates per optimizer; rate selection used validation error. Twenty fresh paired seed blocks gave 320 fits. Every condition continued from 4,096 to 16,384 updates; minimum validation loss within each budget selected test endpoints. Selected and common rates coincided, so these are the same fits. A post hoc sensitivity restarted all conditions with log-conductance bounds $[-20,20]$ and replayed the original $[-7,7]$ bounds, yielding 640 additional trajectories on the same seed blocks. Intervals bootstrap whole seed blocks 20,000 times. At common parameter states, context-gradient cancellation is the norm of the probability-weighted sum of context-conditional distal gradient estimates divided by the weighted sum of their norms, before optimizer transformation. The original monotonic construction and every development outcome remain in Supplementary Figs.~S50--S51.
