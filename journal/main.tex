...
\end{equation}
Excitatory (E) and inhibitory (I) contacts share this factorization (Fig.~\ref{fig:framework}A). Eligibility tells how conductance changes local voltage; compartment credit tells how that voltage changes task loss.
...
Local credit assignment (LocalCA) retains eligibility and supplies an approximate compartment signal, denoted by a hat:
\begin{align}
\widehat\nabla_{g_i}\mathcal L
...
Six rules separated neuronal identity, spatial resolution and learning in the dendritic core (Fig.~\ref{fig:framework}B,D). A strict scalar shared one signal across neurons; neuron-specific feedback broadcast each neuron's own output error through its arbor. A projected one-profile rule averaged the exact activation errors over all twelve nonsomatic compartments, while three subtree profiles averaged them within each proximal compartment and its three children. Both used the same exact field and preserved its mean at identical parameters, while allowing different norms. Exact transport retained every compartment error. A decoder-only reference trained the readout while freezing the core. Three development seeds selected each rule's rate from three common multipliers; ten fresh paired seeds per architecture evaluated selected and original common rates over 180 epochs (Methods; Supplementary Fig.~S7A,B).
...
The richer dictionary nevertheless captured more of the trained activation-error field in the same cohort. Moving from one to three profiles raised mean capture from 0.622 to 0.655 in shunting trees and from 0.764 to 0.859 in additive trees (Fig.~\ref{fig:framework}F). These ratios average over example--neuron fields and use the coordinates delivered during learning. Thus improved reconstruction did not yield a comparable accuracy benefit. Earlier voltage-credit diagnostics and recipient-permutation controls support spatial field structure and the importance of neuronal identity, respectively (Supplementary Section~S2 and Fig.~S7D--G).

With fixed random soma-level feedback, neuron-specific signals improved over scalar feedback by about eleven points, while exact paths added at most 0.09 points (fifteen paired seeds; Fig.~\ref{fig:framework}G; Supplementary Fig.~S6D,E). This control changes weight transport but retains the single dendritic layer and the same output-error dimensionality. Fashion-MNIST also favored neuronal feedback. On flattened CIFAR-10, exact paths reduced accuracy relative to neuronal feedback by 0.86 points (0.55--1.17), while remaining equivalent to matched backpropagation within the predefined margin (Fig.~\ref{fig:framework}G; Supplementary Fig.~S6A--C).

Thus finer delivery need not improve learning even when the error field has spatial structure. Further depth and task-generator controls are in Supplementary Figs.~S6 and S8. We next introduce tasks that make specific spatial distinctions useful.
...
\textbf{A}, Neuron-specific coefficients $\bm c$, dictionary $A$ and diagonal gains $\Gamma$ give credit $\bm\varepsilon=A\Gamma\bm c$. Updates multiply credit by local eligibility and negative learning rate.
\textbf{B}, Twelve nonsomatic sites with $K=1$ common, $K=3$ subtree or $K=12$ site profiles. Projected rules use oracle activation-error coefficients and retain exact somatic errors.
\textbf{C}, Illustrative projection of a unit-energy gradient under isotropic noise variance $\sigma^2$. A rank-$K$ span retains energy $q$. Twice the smoothness constant $L$ times the optimized one-step bound is $q^2/(q+K\sigma^2)$, comparing $(K,q)=(1,0.8)$ and $(2,1)$. This local bound does not predict final structure.
\textbf{D}, MNIST with one dendritic layer and a linear readout. Per-neuron credit broadcasts exact somatic activation errors; decoder-only freezes the core. Lines pair ten fresh seeds per architecture. Three development seeds selected rates; validation selected checkpoints within 180 epochs.
\textbf{E}, Paired effects of additional spatial resolution.
\textbf{F}, Initial and trained capture in the same exact-rule seeds averages squared-energy ratios over nonzero fields from 2,048 images and 128 neurons. Solid triangles and dashed diamonds denote $K=1$ and $K=3$; architecture colors match \textbf{D}.
\textbf{G}, Separate MNIST direct-feedback-alignment (DFA; fifteen seeds) and flattened CIFAR-10 (twenty seeds) controls. Symbols and intervals in \textbf{D--G} show means and descriptive 95\% seed-bootstrap intervals. pp, percentage points.}
\label{fig:framework}
\end{figure}
...
With centered, symmetric class evidence, the shared-mode prediction crosses zero at \(\chi_c=B/[2(B-1)]\): 1, \(2/3\) and \(4/7\) for two, four and eight branches (Fig.~\ref{fig:branchconflict}C; Methods). More branches predict earlier loss of compatibility; this mean-field boundary need not equal a trained chance crossing.
...
The shared rule reached chance at lower conflict as branch count increased, consistent with the predicted shift (Fig.~\ref{fig:branchconflict}F); five two-branch seeds never crossed in the sampled range. Gradient alignment also changed during learning (Methods; Supplementary Fig.~S9C--E). Credit reaching inactive branches can therefore harm learning. A separate two-stream interference task tests this principle (Supplementary Fig.~S9A,B).
...
Flat, grouped-point and gated-point representations receiving the same field behaved identically. Their equivalence locates the conditional advantage in feedback organization and supplies no independent confirmation or evidence for a particular physical carrier.

A separate twenty-seed cohort tested how supplied context cues could set the coefficients (Fig.~\ref{fig:subtreefactorial}D). An encoder trained on 256 calibration cues with noise SD 0.5 and their four-route activation targets improved task accuracy over a frozen profile by 7.16 points, but remained 54.47 points below oracle routing. Exploratory delivery of only its most probable route reduced this noisy-cue gap to 16.26 points. With sixteen noise-free cues, the correct route was already identified: soft coefficients still lost 60.90 points, whereas hard selection matched the oracle in all twenty seeds. Leakage into incompatible routes therefore mattered even with correct identification. Hard selection retained a deficit under noisy cues, and delaying either readout across independent contexts removed its benefit (Supplementary Fig.~S10B--D and Section~S3).
...
\textbf{A}, The task-selected stream carries label sign $s_i$; distractor coefficients are shown on the tree.
\textbf{B}, Raw class-signal sums within cued ancestry groups. Unit-row normalization divides by square root of group size, preserving the designed sign change.
\textbf{C}, Accuracy for ancestry, derangement and the per-seed maximum over four matched controls, computed after training. Dashed line: chance.
\textbf{D}, Separate coefficient-learning cohort: soft and maximum-probability hard readouts relative to oracle routes, with supplied calibration cues and route-activation targets. Twenty seeds are paired within each condition. The noisy soft comparison is primary; hard readout and the noiseless condition are exploratory. Calibration size and cue noise differ between conditions.
\textbf{E}, Four-route ancestry-minus-best-control differences; Holm-adjusted two-sided Wilcoxon $P=0.0101$ across four budgets.
\textbf{F}, Dense, sparse and depth-interleaved control contrasts on a separate scale. Symbols and bars are means and 95\% seed-bootstrap intervals; small points in \textbf{D--F} show twenty paired seeds in each cohort.}
...
The calibrated profile learned pairwise targets but remained inaccurate on quartic and nested targets (Fig.~\ref{fig:prospective}B--D). At the primary 1,024-step checkpoint, mean normalized mean squared errors (NMSE) for exact and calibrated credit were 0.0317 and 0.0236 on pairwise targets, 0.1516 and 0.9813 on quartic targets, and 0.0230 and 0.7690 on nested targets. NMSE divides squared prediction error by clean target variance. The predefined quartic-minus-pairwise difference in calibrated-minus-exact gaps was 0.838 (95\% paired interval, 0.719--0.931), positive in all twenty seeds; a common learning rate gave 0.843 (0.727--0.935). Exact quartic learning was heterogeneous at this checkpoint: seventeen fits were near the noise floor and three remained stalled. The lower pairwise mean for calibrated credit likewise reflected one slow exact fit; exact credit performed better in fifteen of twenty seeds (Supplementary Figs.~S15--S16).

A balanced continuation of every algebraic trajectory to 16,384 updates tested whether these gaps persisted without retuning. All twenty selected-rate Adam exact quartic fits reached the noise floor by 4,096 updates, and the pairwise mean lag disappeared. At 16,384 updates, the quartic-minus-pairwise calibrated-credit deficit was 0.952 (95\% paired interval, 0.883--1.012) at terminal states and 0.790 (0.691--0.880) under validation-based selection, positive in all twenty seeds (Fig.~\ref{fig:prospective}E). Fixed-profile arms contacted coefficient bounds; exact arms did not. These are the same previously observed seeds and bounded dynamics, with common-rate, per-seed and optimizer controls (Supplementary Figs.~S15--S16 and Table~S8).
...
The learned credit spectra help explain these input-matched targets' different tolerance of fixed profiles. At fixed weights and inputs, path derivatives are target-independent; their spectrum describes learned states and coordinates, not a task-only initialization diagnostic. The results favor example-dependent spatial credit without establishing that all six exact channels are necessary.
...
Conductance trees provide a biophysical model for testing this distinction. All rules learned a seven-compartment teacher with monotonic branch responses: extended Adam training kept broadcast NMSE below 0.0006 despite a varying path field (Supplementary Fig.~S18). A separate construction tested context-selected branches with aligned or opposing feature tuning (Fig.~\ref{fig:conductancecredit}A,B).
...
The controls identify what the gate must preserve. Reversing its distal selection gave opposed-task NMSE 0.949. Applying it at proximal compartments as well gave 0.123 and prevented both proximal inhibitory gains from learning: their eligibility is nonzero only during inhibition, precisely when that gate closes (Fig.~\ref{fig:conductancecredit}F). The proper local rule changed all 24 parameters and succeeded across the three tested rates. A continuous shunt-proportional distal gate reached NMSE $2.32\times10^{-5}$ under the same primary rate and budget (Fig.~\ref{fig:conductancecredit}F). Both gates were tested with binary inhibitory contexts; continuously varying or noisy context remains untested.

The gate coarsens the exact rule's strong context-dependent gain ratio. Same-state diagnostics showed that broadcast mixes opposing context-conditioned updates, despite nonnegative per-example alignment with exact gradients (Supplementary Section~S5 and Source Data). This supplies a local implementation given inhibitory context and somatic error, while leaving their biological origin open. The comparisons remain budget- and parameter-bounded. Exact and local-gate fits did not reach conductance bounds, whereas most opposed broadcast fits did; the earlier wide-bound and rate controls remain in Supplementary Fig.~S19.
...
\textbf{F}, Opposed-task outcomes within the primary window for exact credit, hard and continuous distal gates, swapped selection and proximal gating. The continuous gate uses $1/(1+g_p^{\rm I}a_p)$ with the same binary inhibitory activity $a_p$ as the hard rule. Gating proximal credit freezes two inhibitory gains. Points show every seed; larger symbols and bands in \textbf{C--F} show means and descriptive 95\% whole-seed bootstrap intervals. All rates and nine rules are retained in Supplementary Fig.~S20. Earlier exploratory gate runs are excluded.}
...
Credit comparisons within this useful forward architecture depended on training duration. Ten paired seeds compared D1 exact BP with five D3 credit/optimization conditions for up to 600 epochs, retaining the original rates, parameter groups and thirty-epoch validation patience. SI reports restart agreement and complete validation histories. At validation-selected states from the same trajectories, exact-BP D3 exceeded D1 by 30.82 points at 180 epochs and 38.17 at 600 (Fig.~\ref{fig:physicaldepth}D). Eight of ten D1 references stopped early; all fifty D3 fits reached the cap with validation loss still falling. The forward benefit is therefore budget-specific, with unequal optimization maturity across depths.
...
\textbf{D}, Nested-task D1 $[8]$ and D3 $[2,1,2]$ trajectories at matched resources. Validation-selected states are carried forward after stopping. The annotation counts D1 fits still training: seven of ten at epochs 180 and 400, and two at 600.
...
Each inhibitory-bearing segment supplied a candidate route over its input-bearing descendants (Fig.~\ref{fig:topology}A). These supports form dictionary columns, supplemented by a constant column for common broadcast (Fig.~\ref{fig:topology}B). For a site-by-route matrix $A_K$, diagonal excitatory-area weights $W$ and weighted least-squares projector $P_{A_K,W}$, capture of a field $\bm t$ was
\begin{equation}
...
\textbf{D}, Paired ancestry advantages at $K=8$. Surrogate trees preserve segment depths and parent out-degrees; shuffling preserves column nonzero counts. Randomized controls average 200 draws per cell. Small points on the surrogate comparison show all 47 within-cell differences: 38 positive and nine negative.
...
A focal shunt provides an exact link between anatomical routes and conductance-dependent gains. We compared a focal inhibitory shunt with a current injection matching its baseline focal current in reconstructed reciprocal cables. A separately solved somatic current restored baseline somatic voltage after either intervention. The endpoint was the change in the modeled excitatory-conductance gradient.
...
We compared descendants \(D(k)\) with depth-matched off-route sites \(C(k)\); Supplementary Section~S8 gives a kernel criterion for stronger absolute adjoint changes below a shunt, with within-cell, reversal-potential and contact-label controls in Supplementary Fig.~S28B,C.
...
\textbf{A}, Ancestry--response partial rank correlations after controlling Euclidean separation and contact-depth differences. Points show seven targets; diamonds and intervals show means and 95\% target-bootstrap intervals ($-0.069\,[-0.251, 0.106]$ for selected scans and $-0.048\,[-0.224, 0.118]$ for all eligible scans). Selected scans contain 69 partners; all eligible scans comprise thirteen scans of the same targets.
...
In $A\Gamma$, nonzero diagonal gains preserve the dictionary span and oracle capture while changing coefficients and conditioning. This describes a passive shunt in its baseline-weighted ancestry partition, not arbitrary overlapping dictionaries. Stage count, feedback rank, external error dimension and state-dependent path resolution remain distinct (Supplementary Tables~S1--S2).

\subsection*{Image feedback ladders and checkpoint diagnostics}

The trained decoder supplies somatic activation errors $\delta_{i,u}=\partial\mathcal L/\partial a_{i,0,u}$ for example $i$ and neuron $u$. Strict scalar feedback uses their mean, or their largest-magnitude signed coordinate when the mean's magnitude is at most $10^{-6}$, throughout the core including somatic gates. Neuron-specific feedback preserves each coordinate; exact transport additionally applies local path derivatives. The historical matched-width fallback retained neuronal identity at the 128-wide somatic stage, affecting 640 of 2,413,312 core parameters. A separately trained strict-scalar control tests that exception (Supplementary Section~S2 and Source Data indexed in Table~S12).
...
The branch-conflict task uses Eqs.~\ref{eq:branchconflictforward}--\ref{eq:branchconflictupdates}, with independent opposite-class replacements of nonselected Fashion-MNIST inputs at probability $\chi$. Twenty paired seeds cross $B=2,4,8$ with eight doses. Gated-point and BP calculations receive equivalent updates; cyclic derangement changes recipients at fixed rank and support.
...
Synthetic quadratic screens use sixteen Haar coordinates and a designed signal/noise crossover, without a forward neuron (Supplementary Fig.~S2). Retrospective factorial utility uses actual full-batch updates and a global logistic-curvature bound; maximum bandwidth matches its selection success. Supplementary Figs.~S33--S34 retain the failed 12,800-fit prospective selector, its loss to fixed maximum-budget and rank-only baselines in every seed, and the 25,600-fit dynamics extension with evolving noise, observed-context-count controls, sealed choices and computational costs.
...
Original cohorts use ten paired seeds, a 180-epoch cap, thirty-epoch patience and validation-based state selection. BP and LocalCA differ in rates and parameter groups; exact-path LocalCA still computes exact selected gradients. Source checks of one hundred original trajectories verified configurations and final metrics: all fifty D3 fits across five credit/optimizer arms reached the cap with falling validation loss. Sixty source-consistent restarts extend these fifty D3 fits and ten exact-BP D1 references to 600 epochs, selecting minimum-validation-loss states within each budget. All D3 fits and two D1 references reach the larger cap; eight D1 references stop early. D3 selected states lie near the cap while validation loss continues falling. Supplementary Section~S6 retains complete histories, paired accuracy/loss contrasts and source-concordance discrepancies.
