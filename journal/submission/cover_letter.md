...
The experiments map the theory’s boundary. Neuron-specific feedback recovers most of the scalar-feedback deficit, whereas exact path transport adds little on ordinary MNIST, Fashion-MNIST and flattened CIFAR-10. We then vary branch conflict continuously in a controlled Fashion-MNIST family. Shared and path-resolved credit are equivalent when branch eligibilities agree; at full conflict, path-selective information adds 35–58 percentage points. The trained failure threshold shifts with branch count almost exactly as predicted by the shared-mode compatibility calculation. A matched gated-point model reproduces the result, identifying a need for branch-selective information rather than a uniquely dendritic material advantage. Nested ancestry is competitive with matched non-anatomical routes only at intermediate bandwidth.
