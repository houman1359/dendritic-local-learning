...
\textbf{(C)}~Error-broadcast choices. Rank-1, neuron-wise, rank-$K$, and path-structured broadcasts are local approximations with increasing bandwidth; transported error is an analysis oracle that supplies the exact path-specific signal.}
...
Figure~\ref{fig:mechanistic} provides the paper's central empirical chain: shunting changes the dendritic path-gain field, makes the exact compartment-error field more compressible, improves broadcast fidelity, and then improves local learning when the transported-error bottleneck is removed.

Panel~A visualizes the path-gain field on a representative dendritic tree. At matched inhibition ($N_I{=}5$), the measured MNIST path-gain CV is about $1.16$ for additive integration and $0.23$ for shunting, so the shunting tree has a much narrower conductance-stage gain range.
Panel~B measures compressibility directly on the exact compartment-error matrix. For MNIST, the best rank-$1$ residual falls from $0.83\pm0.07$ in additive networks to $0.65\pm0.18$ in shunting networks, and the participation-rank estimate falls from $8.5\pm3.3$ to $3.2\pm1.2$. This is the diagnostic matched to the paper's main claim: rank-$1$ broadcast works when the exact error field is closer to low rank.

Panel~C then measures the sole non-local term from Corollary~\ref{cor:factorization}, $\partial L/\partial V_n$, directly.
On the noise-resilience sweep, rank-$1$ broadcast tracks the exact compartment error better under shunting once inhibition is present: additive remains around $0.14$--$0.19$ across the inhibited regimes, while shunting rises to about $0.20$--$0.24$.
The transported-error oracle obtained by recursively applying conductance-stage path gains improves fidelity sharply, reaching about $0.95$--$0.99$ for additive and essentially $1.0$ for shunting once inhibition is present.

Panel~D trains the same local rule using that oracle signal.
On noise resilience, transported broadcast lifts additive learning to about $92$--$94\%$ and shunting to about $94$--$95\%$ across inhibited regimes.
On MNIST, transported broadcast reaches about $95.4$--$96.6\%$ for additive and $96.5$--$97.3\%$ for shunting.
Appendix Fig.~\ref{fig:cifar10_mechanism_extension} reports the same ladder on flattened CIFAR-10 as a harder-data stress test: shunting rises from $29.7\%$ under rank-$1$ broadcast to $47.2\%$ under transported error, nearly matching its $49.5\%$ backprop ceiling.
The same sweep shows that learned inhibitory conductance is not a cosmetic addition: removing I-to-E conductance lowers shunting backprop from $49.5\%$ to $43.9\%$ and transported LocalCA from $47.2\%$ to $38.6\%$.
We use CIFAR-10 diagnostically rather than as a hard-vision benchmark claim.

Appendix controls fill in the boundaries: coarse quantized broadcasts preserve performance until sign-only or sparse broadcasts remove too much information (Fig.~\ref{fig:low_bandwidth_appendix}); rank-$K$ and path-propagation feedback close part of the transported-oracle gap (Table~\ref{tab:low_rank_broadcast}); additive normalization and DFA help but do not match the shunting conductance model (Figs.~\ref{fig:additive_norm_control}, \ref{fig:fa_dfa_appendix}); explicit inhibitory cells can generate useful path-gain structure in a one-layer input-mode probe (Fig.~\ref{fig:input_mode_path_gain}, Table~\ref{tab:input_mode_probe}); and post-training inhibitory-conductance interventions plus exact-error SVD diagnostics show that learned inhibition is causally task-structured and makes the exact compartment-error field more compressible (Fig.~\ref{fig:inhibition_causality_error_rank}).
...
\textbf{(C)}~Direct compartment-error fidelity on noise resilience. Solid lines compare rank-$1$ broadcast to the exact compartment error; dashed lines show the transported-error oracle.
\textbf{(D)}~Learning with the transported-error oracle. Improving the broadcast field recovers near-ceiling local learning, especially where rank-$1$ broadcast fails.}
...
The inhibitory regime in which shunting helps most is not fixed across dendritic trees.
In the morphology and inhibition sweep (Fig.~\ref{fig:morphology_ie_regime}), the average shunting advantage peaks at higher inhibition for depth-2 trees ($N_I{=}40$ in this sweep) and at intermediate inhibition for depth-3 trees ($N_I{=}10$).
The best individual cells are also morphology-specific, with $[2,2,2]$ peaking at $N_I{=}10$, $[3,3,3]$ at $N_I{=}20$, and the shallower $[2,2]$ tree at $N_I{=}40$.
This supports the broader picture that tree geometry changes how inhibitory conductance shapes effective compartment sensitivities.
...
A routed Double-CIFAR pilot was not included as evidence. The additive-only completed conditions remained near chance ($9.9$--$11.0\%$), and the shunting configurations were rejected by the nonnegative-drive validator because the transfer stage produced signed conductance-driving inputs. We therefore treat that pilot as a configuration screen, not as a scientific comparison.
...
\paragraph{Causal inhibition and exact-error rank.}
...
\caption{\textbf{Causal inhibition and exact-error compressibility.}
