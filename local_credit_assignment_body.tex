...
\textbf{(A)}~Eligibility-rule ablation: 3F is the theorem-derived local eligibility, 4F adds the branch--soma relevance proxy, and 5F adds the confidence wrapper that carries most practical performance.
\textbf{(B)}~Error-source ablation: the rank-$1$ shared somatic error works, whereas a local-mismatch surrogate collapses learning.
\textbf{(C)}~Feedback-bandwidth ladder on noise resilience: R1 is rank-$1$ broadcast, Path is recursive path-propagation, K2/K4/K8 are random low-rank broadcasts, and Or.\ is exact effective transported error.
\textbf{(D)}~Cue routing tests branch-specific credit: R1 is rank-$1$, K1/K2/K4 are random low-rank broadcasts, and PV is pathway-vector feedback. Higher rank helps, but PV does not yet beat the best unstructured low-rank control.}
