...
The main text uses MNIST, Fashion-MNIST \cite{xiao2017fashionmnist}, context gating, noise resilience, and a compressed morphology-regime summary; task schematics, CIFAR-10, and routed-feedback diagnostics are reported in the appendix (Figs.~\ref{fig:task_schematics}, \ref{fig:cifar10_mechanism_extension}, \ref{fig:cue_routing}).
...
\textbf{(D)}~Additive normalization control: normalization partially improves additive LocalCA but does not reach the shunting reference, showing that the shunting advantage is not just ordinary voltage normalization. Error bars denote $\pm$1 s.d.\ across seeds where available.}
\label{fig:competence_regime}
\end{figure*}

In the capacity-calibrated regime, shunting LocalCA stays within about $5$--$6$ percentage points of matched backpropagation on MNIST, Fashion-MNIST, and context gating (Table~\ref{tab:gap_closing}; Fig.~\ref{fig:competence_regime}). Figure~\ref{fig:competence_regime}D addresses a direct normalization-control concern: additive normalization improves additive LocalCA but remains below the shunting reference, so the gain is not explained by generic voltage normalization alone. Broader rule and feedback-design controls are summarized in Appendix Fig.~\ref{fig:rule_feedback_design}, where 5F is the strongest practical wrapper and 3F/4F remain instructive but weak optimizers; matched DFA baselines remain much weaker than shunting LocalCA (Appendix Fig.~\ref{fig:fa_dfa_appendix}).
...
The appendix is organized by how directly each result supports the main mechanism. The first block collects the most useful controls for reading the paper: rule and feedback design axes, synthetic task schematics, and the component-level evidence map. The second block reports competence checks, gradient dynamics, depth/noise stress tests, and baseline comparisons. The third block contains harder-data, morphology, inhibition, and pathway-feedback diagnostics. The final blocks give implementation, theory, and secondary biological weight analyses. Table~\ref{tab:component_carry} is the quickest map of which components carry each result.
...
\caption{\textbf{Synthetic benchmark schematics.}
\textbf{(A)}~Context gating: the right half carries the MNIST digit; the left half is filled with low-amplitude uniform noise.
\textbf{(B)}~Noise resilience: MNIST image corrupted by structured channel noise (fixed 50-D latent projection, $\sigma_{\mathrm{task}}=1.5$, clipped to $[0,1]$).
\textbf{(C)}~Cue integration: a context cue selects which of two noisy cue streams is reliable; dendritic integration routes the reliable cue to the binary readout.
\textbf{(D)}~Fashion-MNIST: 10-class apparel classification sharing the MNIST input shape.}
...
\caption{\textbf{Coarse shared broadcast still works.} Full-precision, $8$-bit, $4$-bit, and $2$-bit quantized broadcasts reach similar accuracy ($61$--$62\%$; $p>0.9$ for $4$-bit vs.\ full), while sign-only and sparse top-$30\%$ broadcasts degrade. This supports the low-bandwidth-broadcast framing: useful information is mostly carried by coarse direction rather than fine magnitude.}
...
\section{Secondary Biological Weight Analyses}
...
\subsection*{Weight Distribution Sensitivity Analysis}
