...
\textbf{(C)}~Path gain through the tree. Exact backprop supplies a path-specific compartment error; rank-1 LocalCA replaces that field with one shared broadcast.
\textbf{(D)}~Broadcast approximations. Rank-1 shared broadcast, neuron-wise broadcast, rank-$K$ random broadcast, path-structured broadcast, and transported oracle form a ladder from low-bandwidth local learning to an exact non-local upper bound.}
