...
\textbf{(B)}~Exact credit is path-specific, whereas rank-1 LocalCA broadcasts one shared error field.
\textbf{(C)}~LocalCA keeps the exact synapse-local eligibility and substitutes $e_n\!\approx\!\delta_n$; rank-1, neuron-wise, rank-$K$, and path-structured broadcasts trade biological bandwidth for fidelity, while transported error is an oracle.}
...
\caption{\textbf{Gradient-fidelity diagnostic.} \textbf{(A)}~Exact-factorization sanity check: reconstructed gradients match autograd to numerical precision. \textbf{(B)}~Checkpointed final-state weighted cosine similarity between local and backprop gradients. \textbf{(C)}~Final-state scale mismatch, $|\log_{10}(\|g^{\mathrm{local}}\|/\|g^{\mathrm{bp}}\|)|$. Error bars are $\pm$1 s.d.\ across 3 checkpointed seeds; main learning and oracle sweeps use 5 seeds unless noted. \textbf{(D)}~Alignment over learning; Appendix Fig.~\ref{fig:alignment_norm_dynamics} shows finite gradient norms.}
...
\textbf{(A)}~Matched backprop ceilings vs.\ rank-$1$ 5F LocalCA on MNIST, Fashion-MNIST, and context gating.
...
In the capacity-calibrated regime, shunting LocalCA stays within about $5$--$6$ percentage points of matched backpropagation on MNIST, Fashion-MNIST, and context gating (Table~\ref{tab:gap_closing}; Fig.~\ref{fig:competence_regime}). Figure~\ref{fig:competence_regime}D shows that additive normalization improves additive LocalCA but remains below the shunting reference, so the gain is not explained by generic voltage normalization alone. Figure~\ref{fig:rule_feedback_controls} summarizes the rule and feedback controls: 5F is the strongest practical wrapper, somatic rank-$1$ error is essential, and higher feedback bandwidth improves the noise-resilience diagnostic.
...
The main result is a condition for faithful dendritic broadcast, not a claim that LocalCA universally replaces backpropagation. Exact conductance-tree credit separates synapse-local eligibility from one non-local compartment error, so local learning succeeds only when a low-dimensional broadcast approximates that path-specific error field.

The experiments support a coherent but bounded mechanism. Shunting inhibition modulates input resistance along credit paths, making the exact error field more compressible for rank-$1$ broadcast in the regimes studied here; transported error and inhibition interventions identify broadcast fidelity and learned inhibitory structure as key bottlenecks. At the same time, 5F is an empirical stabilizer, Eq.~\eqref{eq:compression_condition} is interpretive, activation shape matters, routed and harder-data settings require richer feedback, and the present models use input-driven shunting rather than recurrent lateral inhibition. The broader connection is between a biophysical operation, shunting control of input resistance, and a machine-learning object: the low-rank structure of the error field.

\begin{nolinenumbers}
\begin{center}
\refstepcounter{figure}\label{fig:rule_feedback_controls}
\centering
\includegraphics[width=\textwidth]{fig5_rule_feedback_controls.pdf}
\vspace{-0.8ex}
\parbox{\textwidth}{\small\textbf{Figure~\thefigure: Rule and feedback controls.}
