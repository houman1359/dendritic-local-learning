...
The fidelity and oracle panels test the same bottleneck directly. On noise resilience, rank-$1$ broadcast tracks the exact compartment error better under shunting once inhibition is present (about $0.20$--$0.24$ vs.\ $0.14$--$0.19$ for additive), while transported error reaches about $0.95$--$1.0$ fidelity. Training with transported error lifts additive learning to $92$--$94\%$ and shunting to $94$--$95\%$ on noise resilience, and reaches $95.4$--$97.3\%$ on MNIST across both cores. The CIFAR-10 appendix stress test shows the same ladder in a non-saturated setting: shunting rises from $29.7\%$ under rank-$1$ broadcast to $47.2\%$ under transported error, near its $49.5\%$ backprop ceiling (Fig.~\ref{fig:cifar10_mechanism_extension}). Finally, the one-layer inhibitory-path probe shows that exact transport closes the direct-I rank-$1$ gap ($85.2\%\to95.2\%$), and explicit inhibitory dendritic cells trained by the same local rule remain near that ceiling ($94.2\%$).
...
In the capacity-calibrated regime, shunting LocalCA stays within about $5$--$6$ percentage points of matched backpropagation on MNIST, Fashion-MNIST, and context gating (Table~\ref{tab:gap_closing}; Fig.~\ref{fig:competence_regime}). Figure~\ref{fig:competence_regime}D keeps the rule-family ablation in the main text: 5F is the strongest practical wrapper but remains heuristic, while 3F/4F are instructive but weak optimizers. The broader rule and feedback-design controls, including local-mismatch, path-propagation, low-rank, and cue-routing variants, are summarized in Appendix Fig.~\ref{fig:rule_feedback_design}. Classical random-feedback baselines are much weaker on the matched MNIST family, with DFA at $66.1\%$ on shunting and $62.2\%$ on additive versus $91.4\%$ for the best shunting LocalCA result (Appendix Fig.~\ref{fig:fa_dfa_appendix}).

\subsection{Regime Dependence of the Shunting Advantage}

With matched rank-$1$ broadcast, shunting is modestly better on MNIST, Fashion-MNIST, and context gating, including $0.803\pm0.006$ vs.\ $0.787\pm0.007$ for additive on context gating. The gap grows on inhibition-sensitive noise resilience once inhibitory conductance is present (Fig.~\ref{fig:competence_regime}), consistent with the view that shunting helps most when input resistance is part of the useful credit geometry.

\section{Discussion and Limitations}
\label{sec:discussion}
The main result is a condition for faithful dendritic broadcast, not a claim that LocalCA universally replaces backpropagation. Rank-$1$ broadcast succeeds when the exact compartment-error field is sufficiently compressible, and shunting can help by changing input resistance along credit paths. The strongest causal evidence is that transported error largely closes the local-learning gap, while destroying learned inhibition collapses performance even when total conductance scale is preserved.

Boundaries remain. 5F is an empirical stabilizer, Eq.~\eqref{eq:compression_condition} is an interpretive condition rather than a positive headline diagnostic, and routed tasks/CIFAR-10 expose where scalar broadcast needs higher-rank or transported signals. Broader impacts are indirect: the work may aid mechanistic neuroscience and efficient local-learning design, but overclaiming biological realism or robustness would be misleading; we mitigate this by stating assumptions, negative controls, and failure modes explicitly.
