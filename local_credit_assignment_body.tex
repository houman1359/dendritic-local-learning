...
\caption{\textbf{Soma-on extension for LocalCA.} Enabling direct somatic input (\texttt{somatic\_synapses=true}) improves mean test accuracy across all four paper-facing LocalCA families. The largest gain appears in morphology scaling, where the mean rises from $35.8\%$ to $70.7\%$ and the best run rises from $57.0\%$ to $76.4\%$. Gains in the shunting-regime and error-shaping families are smaller but still substantial. Delta labels show soma-on minus soma-off. Because the main mechanistic analysis was developed in the dendrite-only feedforward regime, we report soma-on as a supportive extension rather than replacing the main operating regime.}
...
\bottomrule
\end{tabular}
\caption{\textbf{Representative paired soma-on deltas across the four paper-facing LocalCA phase families.} Values compare matched safeguarded reruns with and without \texttt{somatic\_synapses} and report both the change in mean test accuracy and the change in best-seed test accuracy. Cue-routing soma and CIFAR-10 depth-4 soma are reported separately in Fig.~\ref{fig:cue_routing_soma} and Table~\ref{tab:cifar10_depth4_soma_extension} because those extensions are summarized on a different stable subset or task family.}
...
\centering\small
\renewcommand{\arraystretch}{1.06}
\begin{tabular}{@{}L{0.24\linewidth}L{0.30\linewidth}L{0.12\linewidth}L{0.22\linewidth}@{}}
\toprule
\textbf{Figure / Table} & \textbf{Sweep} & \textbf{Seeds} & \textbf{Tier} \\
\midrule
Fig.~\ref{fig:model_schematic}D       & Fig.~1 MNIST learning dynamics       & 5 & Headline \\
Fig.~\ref{fig:gradient_fidelity}      & Gradient-fidelity diagnostic         & 5 & Headline \\
Fig.~\ref{fig:mechanistic}            & Mechanistic chain + oracle + summary & 5 & Headline \\
Fig.~\ref{fig:competence_regime}      & Competence / regime dose-response    & 5 & Headline \\
Fig.~\ref{fig:cifar10_mechanism_extension} / Table~\ref{tab:cifar10_mechanism_extension} & CIFAR-10 compact E/I mechanism extension & 5 & Headline \\
Fig.~\ref{fig:cue_routing}            & Cue-routing (soma off)               & 5 & Supportive \\
Table~\ref{tab:gap_closing}           & Competence gaps (BP ceiling vs.\ 5F) & 5 & Headline \\
\midrule
Fig.~\ref{fig:soma_extension}         & Soma-on extension across phase families & 5 & Supportive \\
Table~\ref{tab:soma_delta_summary}   & Paired soma-on delta summary         & 5 & Supportive \\
Fig.~\ref{fig:bm_policy}              & $(b,m)$ update-policy comparison     & 4 & Supportive \\
Fig.~\ref{fig:ablation}               & Component ablation (MNIST)           & 5 & Supportive \\
Fig.~\ref{fig:cue_routing_soma}       & Cue-routing soma-on (stable subset)  & 5 & Supportive \\
Fig.~\ref{fig:weight_sensitivity}     & Weight-distribution sensitivity      & 5 & Supportive \\
Fig.~\ref{fig:weight_dist_by_depth}   & Weight-distribution vs.\ depth       & 5 & Supportive \\
Fig.~\ref{fig:weight_dist_soma_comparison} & Weight-distribution soma-off vs.\ soma-on & 5 & Supportive \\
Table~\ref{tab:cifar10_depth4_soma_extension} & CIFAR-10 depth-4 soma extension & 3 & Supportive \\
Fig.~\ref{fig:fa_dfa_appendix}        & FA/DFA baseline comparison           & 5 & Supportive \\
Fig.~\ref{fig:low_bandwidth_appendix} & Broadcast-bandwidth / quantization   & 5 & Supportive \\
Fig.~\ref{fig:additive_norm_control}  & Additive + normalization control     & 5 & Supportive \\
Fig.~\ref{fig:additional_stress_tests} / \ref{fig:verification_appendix} & Stress tests; verification & 5 & Supportive \\
Fig.~\ref{fig:five_factor_sensitivity}& 5F sensitivity                       & 3 & Supportive \\
Table~\ref{tab:low_rank_broadcast}    & Rank-bridge                          & 5 & Supportive \\
\midrule
Fig.~\ref{fig:morphology_ie_regime}   & Morphology $\times$ inhibition map   & 3 & Supportive \\
(legacy) info-shunting                & Signed-input stress test             & 1--3 & Exploratory \\
\bottomrule
\end{tabular}
\caption{\textbf{Seed-to-claim map.} Each main-paper and appendix figure or table is pinned to its sweep, seed count, and claim tier (Sec.~\ref{sec:claim_tiers}). The BP ceilings in Table~\ref{tab:gap_closing} come from dedicated matched-capacity $5$-seed standard-training refreshes.}
...
For tasks with latent pathway structure, the broadcast becomes a low-dimensional role vector inferred from the router. Local pathway activity gates that vector, and block-structured transport carries it upward so earlier branches receive feedback aligned with their downstream descendants.
