...
We distinguish five feedback objects: \textbf{rank-1 shared broadcast}, $e_n=\bar{\delta}\mathbf{1}$ with $\bar{\delta}=\mathrm{mean}(\delta_0)$; \textbf{neuron-wise broadcast}, $e_n=\delta_0$ when layer width matches the error dimension; \textbf{rank-$K$ random broadcast}, $c=P_K\delta_0$ and $e_n=\sum_k q_{n,k}c_k$; \textbf{path-structured broadcast}, $e_n=\lambda_{\mathrm{mix}}\bar{\delta}\mathbf{1}+(1-\lambda_{\mathrm{mix}})\Gamma_n(\delta_0)$ using router-inferred pathway roles \cite{nature2026vectorized}; and the \textbf{transported oracle}, $e_n^{\mathrm{transport}}=\alpha_n\delta_0=\partial L/\partial V_n$, an analysis upper bound because it supplies the exact path gain from Theorem~\ref{thm:tree_backprop}.
...
\textbf{(C)}~Feedback-bandwidth ladder on noise resilience: R1 is rank-$1$ broadcast, Path is recursive path-propagation, K2/K4/K8 are random low-rank broadcasts, and Or.\ is exact transported error.
\textbf{(D)}~Cue routing tests branch-specific credit: R1 is rank-$1$, K1/K2/K4 are random low-rank broadcasts, and PV is pathway-vector feedback. Higher rank helps, but the current PV implementation does not yet beat the best unstructured low-rank control.}
