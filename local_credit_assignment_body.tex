...
The empirical story follows this sequence. Figure~\ref{fig:model_schematic} defines the conductance tree and broadcast taxonomy. Figure~\ref{fig:gradient_fidelity} verifies the exact factorization. Figure~\ref{fig:rule_feedback_design} separates the local eligibility rule from the broadcast and tree-propagation choices. Figure~\ref{fig:mechanistic} links path-gain compressibility to broadcast fidelity and learning, and Figure~\ref{fig:competence_regime} shows the practical supervised regimes. Morphology and harder-data experiments are appendix diagnostics rather than headline benchmark claims.
...
This is a structured rank-$K$ broadcast, not a separate learning principle; the appendix gives the router-derived role construction and branch-role plasticity details.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.98\textwidth]{fig4_rule_feedback_design.pdf}
