...
The empirical story follows this sequence. Figure~\ref{fig:model_schematic} defines the conductance tree, exact credit object, and broadcast taxonomy. Figure~\ref{fig:gradient_fidelity} verifies the exact factorization and measures local-backprop fidelity. Figure~\ref{fig:mechanistic} links path-gain compressibility to broadcast fidelity, oracle transport, and learning. Figures~\ref{fig:competence_regime} and \ref{fig:morphology_ie_regime} show the practical supervised regimes and the morphology-dependent inhibitory operating point. Harder-data and routed-feedback experiments are reported as appendix diagnostics rather than headline benchmark claims.
...
\textbf{(C)}~Local rule and feedback choices. The 3F rule contains the exact local eligibility from the theorem; 4F/5F add stabilizing local factors, and the feedback ladder ranges from rank-1 broadcast to transported oracle.}
...
\begin{proposition}[When shunting compresses path gains]
\label{prop:path_gain_compression}
Let $z_n=\log \alpha_n$ be the log path gain for compartment $n$. If added inhibitory conductance contributes a path-dependent attenuation
\begin{equation}
...
\vspace{-4pt}
\begin{proof}
Eq.~\ref{eq:log_gain_attenuation} follows by taking the logarithm of the product ratio in Prop.~\ref{prop:inhibitory_path_gain}. Eq.~\ref{eq:log_gain_variance} is the variance identity for $z-\beta$, and rearranging gives the condition.
\end{proof}

Prop.~\ref{prop:path_gain_compression} is the mechanism we test empirically. Shunting is not assumed to concentrate gains by default; it concentrates or low-rank compresses the credit field only when inhibitory attenuation is aligned with the high-gain paths. This is why the morphology and input-mode diagnostics are reported as operating-regime tests rather than as monotonic ``more inhibition is better'' claims.
...
\item \textbf{Path-structured broadcast:} $e_n = \alpha\,\bar{\delta}\cdot\mathbf{1} + (1{-}\alpha)\,\Gamma_n(\delta_0)$, where $\Gamma_n$ projects the somatic error into router-inferred pathway roles, gates those roles by local pathway activity, and transports the resulting vector hierarchically through the block-structured dendritic tree.
...
Instead of collapsing $\delta_0$ to a rank-1 or neuron-wise broadcast, we project it into this role basis and gate each role by local pathway activity:
\[
...
We evaluate two settings: a \emph{capacity-calibrated} regime, where matched architectures achieve strong backprop performance, and \emph{controlled sweeps} that isolate inhibition strength, broadcast mode, morphology, and architecture.
The main text uses MNIST, Fashion-MNIST \cite{xiao2017fashionmnist}, context gating, and noise resilience; cue integration and CIFAR-10 are reported as appendix diagnostics because they are most useful for probing failure modes rather than establishing the core mechanism.
Formal task definitions are in Appendix~\ref{app:synthetic_tasks}.
...
Most MNIST, Fashion-MNIST, and context-gating results use 5F with the historical \texttt{per\_soma} broadcast implementation.
Because the hidden widths in these feedforward settings exceed the decoder output dimension, that implementation reduces to rank-$1$ shared broadcast in the main mechanism and competence sweeps; higher-rank controls then use explicit low-rank, path-propagation, path-transport, or pathway-vector variants.
...
All dendritic runs supporting the main claims keep synaptic and dendritic conductances nonnegative via positive parameter transforms and enforce a nonnegative first-layer synaptic drive. Image inputs are used in $[0,1]$, noise-resilience corruption is clipped back to $[0,1]$, and context gating uses a nonnegative contextual figure-ground MNIST construction.
The inhibition sweeps additionally use a transported-error oracle derived from Theorem~\ref{thm:tree_backprop} as an upper bound rather than as the proposed biological rule.
Main mechanistic, oracle, competence, morphology, and activation-audit results use $5$ seeds per condition unless otherwise noted. Appendix diagnostics state their seed counts in captions or tables.

The experiments follow three questions. First, does the exact factorization explain the gradient geometry observed in code? Second, does shunting change the path-gain field in the direction predicted by Props.~\ref{prop:inhibitory_path_gain} and \ref{prop:path_gain_compression}, and does exact path transport rescue learning when rank-$1$ broadcast is insufficient? Third, do the same ingredients remain useful in capacity-calibrated supervised tasks and across dendritic morphologies?
...
\caption{\textbf{Gradient-fidelity diagnostic.} \textbf{(A)}~Weighted cosine similarity between local and backprop gradients: shunting (green) is consistently more aligned than additive (blue) on both MNIST and context gating. \textbf{(B)}~Full-vector local and backprop gradient norms over training. Additive gradients remain finite, so the near-zero additive cosine in panel~A reflects a directional mismatch rather than vanishing gradients. \textbf{(C)}~5F alignment dynamics over training. Shunting remains more positively aligned, whereas additive stays near zero. \textbf{(D)}~Exact-factorization sanity check across all theory-diagnostic conditions, reported with relative $L_2$ reconstruction error and scale mismatch. Reconstructed gradients match autograd to numerical precision, showing that LocalCA's only approximation is in the broadcast substitute for the compartment error.}
...
On noise resilience, additive learning rises from $66.8\%$ at $N_I{=}0$ to about $81$--$84\%$ across the inhibited regimes under rank-$1$ broadcast, and transported broadcast lifts those same conditions to about $92$--$94\%$.
Shunting rises from $54.3\%$ at $N_I{=}0$ to about $81$--$86\%$ under rank-$1$ broadcast and to about $94.3$--$95.2\%$ under transported broadcast.
On MNIST, transported broadcast reaches about $95.4$--$96.6\%$ for additive and $96.5$--$97.3\%$ for shunting.
Appendix Fig.~\ref{fig:cifar10_mechanism_extension} reports the same ladder on flattened CIFAR-10 as a harder-data stress test: shunting rises from $28.8\%$ under rank-$1$ broadcast to $47.4\%$ under transported error, nearly matching its $49.5\%$ backprop ceiling.
We use that result diagnostically rather than as a hard-vision benchmark claim.
Together, the oracle results support path-gain fidelity as the dominant bottleneck for LocalCA performance, while the smaller remaining additive--shunting gap under matched transported broadcast suggests an additional contribution from conductance-shaped local sensitivities.

Panel~D collapses the chain across all conditions into a single scatter of rank-$1$ cosine alignment against test accuracy. Shunting cells (squares) cluster at high alignment and high accuracy; additive cells (circles) cluster at the opposite end, and the same monotonic relationship holds on both MNIST and noise resilience. This is a compact confirmation that the (path-gain compressibility $\to$ compartment-error fidelity $\to$ test accuracy) chain is not a dataset-specific artifact.
Appendix controls fill in the boundaries: coarse quantized broadcasts preserve performance until sign-only or sparse broadcasts remove too much information (Fig.~\ref{fig:low_bandwidth_appendix}); rank-$K$ and path-propagation feedback close part of the transported-oracle gap (Table~\ref{tab:low_rank_broadcast}); additive normalization and DFA help but do not match the shunting conductance model (Figs.~\ref{fig:additive_norm_control}, \ref{fig:fa_dfa_appendix}); and explicit inhibitory cells can generate useful path-gain structure in a one-layer input-mode probe (Fig.~\ref{fig:input_mode_path_gain}, Table~\ref{tab:input_mode_probe}).
...
\textbf{(A)}~Conductance-stage path-gain dispersion on MNIST, measured as the coefficient of variation of the exact tree attenuation $\alpha_n$ across compartments. Shunting keeps these conductance-stage path gains much more compressed than the additive control as inhibition increases.
\textbf{(B)}~Direct compartment-error fidelity on noise resilience. Solid lines compare the rank-$1$ broadcast $e_n$ to the exact compartment error $\partial L/\partial V_n$; dashed lines show the transported-error oracle obtained by recursively applying conductance-stage path gains. Under the nonnegative-input setup, shunting still improves the simple rank-$1$ broadcast in the inhibited regimes, while the transported oracle defines the remaining approximation gap and becomes nearly exact once inhibition is present.
\textbf{(C)}~Learning with the transported-error oracle on noise resilience. Improving the broadcast field is enough to recover near-ceiling local learning, especially in the additive architecture where rank-$1$ broadcast fails most severely.
\textbf{(D)}~Global mechanism summary. Rank-$1$ cosine alignment (x-axis) vs.\ test accuracy (y-axis) across all conditions (additive/shunting $\times$ five inhibition levels $\times$ two datasets, $5$ seeds each). Shunting cells (squares) cluster at high alignment and high accuracy; additive cells (circles) cluster at low alignment and low accuracy. The same monotonic chain from~(A) through~(C) collapses cleanly into the (alignment $\to$ accuracy) relationship shown here, and it holds on both MNIST and noise resilience, confirming that the mechanistic chain is not a dataset-specific artifact. The broadcast-bandwidth / quantization control previously shown as panel D is now reported separately as Appendix Fig.~\ref{fig:low_bandwidth_appendix}.}
...
In the capacity-calibrated regime, shunting dendritic cores remain reasonably competitive under local learning: on MNIST, Fashion-MNIST, and context gating, the best local configuration (5F, rank-$1$ broadcast in these architectures, local decoder) stays within about $5$--$6$ percentage points of the matched backprop ceiling (Table~\ref{tab:gap_closing}; Fig.~\ref{fig:competence_regime}).
...
The benefit of shunting varies across tasks. With matched rank-$1$ broadcast, shunting is modestly better on MNIST and Fashion-MNIST, and the gap grows on the inhibition-sensitive noise-resilience benchmark once inhibitory conductance is present (Fig.~\ref{fig:competence_regime}). This is the expected regime dependence: shunting helps most when input resistance is part of the useful credit geometry rather than merely an added conductance load.
...
\textbf{(A)}~Matched backprop ceilings vs.\ the best rank-$1$ 5F LocalCA rule on MNIST, Fashion-MNIST, and context gating.
\textbf{(B,C)}~Inhibitory dose-response and shunting advantage on MNIST and noise resilience.
\textbf{(D)}~Fashion-MNIST confirms a smaller but consistent shunting advantage. Error bars denote $\pm$1 s.d.\ across seeds.}
\label{fig:competence_regime}
\end{figure*}

\subsection{Morphology Shifts the Inhibitory Operating Regime}

The inhibitory regime in which shunting helps most is not fixed across dendritic trees. In the morphology$\times$inhibition sweep (Fig.~\ref{fig:morphology_ie_regime}), depth-2 trees show their largest shunting gains near $N_I{=}0$, whereas depth-3 trees retain their strongest gains around $N_I{=}5$. This supports the broader picture that tree geometry changes how inhibitory conductance shapes effective compartment sensitivities.
...
\textbf{(A,B)}~Additive and shunting LocalCA across dendritic morphologies and inhibitory synapse counts.
\textbf{(C,D)}~The shunting advantage peaks at different inhibition levels for shallow and deeper trees, showing that conductance normalization and morphology interact rather than contributing independently.}
\label{fig:morphology_ie_regime}
\end{figure*}

\paragraph{Scope.}
\label{sec:discussion}
The claim is mechanistic, not an unconstrained benchmark claim: CIFAR-10, routed feedback, and depth scaling are diagnostics of where rank-$1$ broadcast breaks, and shunting is not a universal solution for hard vision or biological learning.
...
\caption{\textbf{CIFAR-10 mechanism stress test in the compact direct-I-stream family.}
\textbf{(A)}~Matched additive vs.\ shunting ladder with a nonlinear decoder and decoder-aware soma-error mapping. Shunting is competitive under standard training ($49.5 \pm 0.7$\%) and transported-error LocalCA nearly recovers that ceiling ($47.4 \pm 0.6$\%), while naive rank-$1$ broadcast remains much weaker.
\textbf{(B)}~Shunting rank bridge on the same family. Random low-rank feedback improves over rank-$1$ broadcast, but exact transported error remains the strongest signal. This figure is a harder-data diagnostic, not a claim of competitive CIFAR-10 benchmarking.}
...
Fig.~\ref{fig:cifar10_mechanism_extension} / Table~\ref{tab:cifar10_mechanism_extension} & CIFAR-10 compact direct-I-stream mechanism extension & 5 seeds & Supportive \\
