\begin{abstract}
Biologically plausible credit assignment requires replacing compartment-specific backpropagation errors with signals available to dendritic synapses. We ask when a low-dimensional somatic broadcast can be a faithful substitute in conductance-based dendritic trees. Exact gradients show that each synaptic update factorizes into a local eligibility term, presynaptic activity times driving force times input resistance, and a single non-local compartment error. Local learning is therefore an approximation to this path-specific error field. We show that shunting inhibition modulates input resistance along dendritic paths and can make the error field more compressible for rank-1 broadcast. Across matched additive and shunting dendritic networks, shunting improves local-backprop gradient alignment and reduces gradient scale mismatch. Transported-error oracle experiments largely close the local-learning gap, identifying broadcast fidelity as the dominant bottleneck. Under nonnegative conductances and rank-1 broadcast, shunting LocalCA stays within about 5--6 percentage points of matched backpropagation ceilings on MNIST, Fashion-MNIST, and context gating; routed and harder-data diagnostics reveal when richer pathway-specific feedback is required. These results identify path-gain compressibility as a mechanistic condition under which simple local credit assignment is effective in dendritic networks.
\end{abstract}
...
The empirical story follows this sequence. Section~\ref{sec:model} derives the conductance-tree credit object, and Section~\ref{sec:local_rules} defines the local approximations. Figure~\ref{fig:model_schematic} defines the conductance tree and broadcast taxonomy. Figure~\ref{fig:gradient_fidelity} verifies the exact factorization. Figure~\ref{fig:rule_feedback_design} separates the local eligibility rule from the broadcast and tree-propagation choices. Experiments in Section~\ref{sec:experiments} then test the mechanism: Figure~\ref{fig:mechanistic} links path-gain compressibility to broadcast fidelity and learning, and Figure~\ref{fig:competence_regime} shows the practical supervised regimes. Section~\ref{sec:discussion} summarizes the boundary conditions. Morphology and harder-data experiments are appendix diagnostics rather than headline benchmark claims.
...
The main text uses MNIST, Fashion-MNIST \cite{xiao2017fashionmnist}, context gating, and noise resilience; CIFAR-10, morphology, and routed-feedback diagnostics are reported in the appendix.
...
\textbf{(E)}~Compact inhibitory-path probe. In a one-layer noise-resilience test, exact path transport closes the rank-$1$ gap, and explicit inhibitory cells trained by the same local rule remain near the explicit-I backprop ceiling. Error bars denote $\pm$1 s.d.\ across seeds.}
...
Appendix Fig.~\ref{fig:cifar10_mechanism_extension} reports the same ladder on flattened CIFAR-10 as a harder-data stress test. In the soma-off compact control, shunting rises from $29.7\%$ under rank-$1$ broadcast to $47.2\%$ under transported error, nearly matching its $49.5\%$ backprop ceiling.
...
Panel~E keeps the inhibitory-path interpretation in the main figure. In a one-dendritic-layer noise-resilience probe, direct input-driven inhibition reaches only $85.2\pm0.7\%$ under rank-$1$ broadcast, but exact path transport reaches $95.2\pm0.0\%$. Replacing the direct inhibitory stream with explicit inhibitory dendritic cells trained by the same local rule remains near that ceiling ($94.2\pm0.2\%$). This does not make the explicit-I probe a headline benchmark, but it shows that an inhibitory population can implement the path-gain gating mechanism when the architecture isolates the dendritic-path question.

Appendix controls fill in the boundaries: coarse quantized broadcasts preserve performance until sign-only or sparse broadcasts remove too much information (Fig.~\ref{fig:low_bandwidth_appendix}); rank-$K$ and path-propagation feedback close part of the transported-oracle gap (Table~\ref{tab:low_rank_broadcast}); additive normalization and DFA help but do not match the shunting conductance model (Figs.~\ref{fig:additive_norm_control}, \ref{fig:fa_dfa_appendix}); the full input-mode probe is reported in Fig.~\ref{fig:input_mode_path_gain} and Table~\ref{tab:input_mode_probe}; and post-training inhibitory-conductance interventions plus exact-error SVD diagnostics show that learned inhibition is causally task-structured and makes the exact compartment-error field more compressible (Fig.~\ref{fig:inhibition_causality_error_rank}).
...
\textbf{(A)}~Matched backprop ceilings vs.\ rank-$1$ 5F LocalCA on MNIST, Fashion-MNIST, and context gating, including the matched additive context-gating rerun.
...
The benefit of shunting varies across tasks. With matched rank-$1$ broadcast, shunting is modestly better on MNIST, Fashion-MNIST, and context gating; the matched additive context-gating run reaches $0.787\pm0.007$ versus $0.803\pm0.006$ for shunting. The gap grows on the inhibition-sensitive noise-resilience benchmark once inhibitory conductance is present (Fig.~\ref{fig:competence_regime}). This is the expected regime dependence: shunting helps most when input resistance is part of the useful credit geometry rather than merely an added conductance load.

\section{Discussion and Limitations}
\label{sec:discussion}
The main result is a condition for faithful dendritic broadcast, not a claim that the present LocalCA optimizer is a universal replacement for backpropagation. Exact conductance-tree credit factorizes into synapse-local eligibility and a path-specific compartment error. Rank-$1$ broadcast succeeds when that exact error field is sufficiently compressible, and shunting inhibition can help by changing input resistance along the paths that carry error back to distal synapses. The transported-error oracle and inhibitory-intervention controls are the strongest causal evidence for this interpretation: improving the compartment-error field largely closes the local-learning gap, while destroying learned inhibition collapses performance even when total conductance scale is preserved.

Several boundaries remain. The practical 5F rule is an empirical stabilizer rather than a theorem-derived update; 4F alone gives little gain over 3F. Eq.~\eqref{eq:compression_condition} clarifies when inhibition should narrow log path gains, but the empirical covariance margin is not used as a positive headline result. Routed tasks and CIFAR-10 expose the limits of scalar broadcast: higher-rank or transported signals can help, but the current pathway-vector design is not yet a solved biologically structured replacement. These limitations are useful design constraints for future local learners with richer dendritic teaching channels.
...
\textbf{(A)} Strictly nonnegative cue-routing controls saturate near $99\%$ for learned-I and no-I variants, so this rerun is not a performance separation.
