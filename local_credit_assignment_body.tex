...
Appendix Table~\ref{tab:shunting_additive_mechanism} summarizes the matched shunting/additive mechanisms.
...
(A) Path-gain fields at matched inhibition. (B) Exact-error compressibility on MNIST. (C) Post-training inhibitory-conductance interventions (L: learned; 0/sh/m/u: zero, shuffled, mean-clamped, uniform matched). (D) Compartment-error fidelity on noise resilience; dashed lines show the \textbf{[C]} transported-error oracle. (E) Transported-error learning where rank-1 broadcast fails. Error bars are $\pm$1 s.d.}
...
\begin{tabular}{@{}L{0.19\linewidth}L{0.36\linewidth}L{0.36\linewidth}@{}}
\toprule
...
Driving force & Present through $E-V$ & Absent from the additive derivative \\
Input resistance & State-dependent, $R^{\mathrm{tot}}=(g^{\mathrm{tot}})^{-1}$ & Effectively fixed by definition \\
...
Implementation & Same branch-layer forward path with conductance denominator & Same branch-layer forward path with additive integration \\
\bottomrule
\end{tabular}
\caption{\textbf{Architecture-matched shunting and additive comparison.} Both cores use the same tree, synapse banks, nonnegative conductance parameters, and optimizer protocol; only the branch-level integration rule and its matched local derivative differ.}
