...
\textbf{(A)}~A compartmental dendritic neuron with excitatory (blue triangles) and inhibitory (red circles) synapses. Voltages propagate to the soma through learned dendritic conductances (green arrows). A somatic broadcast error $e_n$ (dashed red) reaches all compartments, and each synapse combines it with local quantities such as presynaptic drive $x_j$, input resistance $R_n^{\mathrm{tot}}$, and driving force $E_j{-}V_n$.
\textbf{(B)}~LocalCA rule hierarchy. 3F is the theorem-facing rule, 4F adds the heuristic morphology-aware factor $\rho$, and 5F adds the heuristic confidence-like factor $\phi$.
\textbf{(C)}~Broadcast modes. Scalar sends one shared field, per-soma sends an output-space vector when dimensions match and otherwise falls back to scalar, random low-rank adds a few unstructured channels, and structured pathway feedback preserves pathway identity when branches should receive different teaching signals.
\textbf{(D)}~Representative MNIST learning dynamics from the fixed-epoch Figure 1 sweep. Shunting with the stronger local wrappers tracks backprop much more closely than the additive control, giving a concrete preview before the paper turns to the sharper gradient-fidelity diagnostics in Fig.~\ref{fig:gradient_fidelity}.}
...
\caption{\textbf{Mechanistic evidence: $A \to B \to C \to D$.}
...
\end{tabular}
\renewcommand{\arraystretch}{1.0}
\caption{\textbf{Training hyperparameters by experiment.} All runs use Adam with default $\beta_1{=}0.9$, $\beta_2{=}0.999$, $\varepsilon{=}10^{-8}$. Reactivation and block-linear parameter groups use a slower LR to keep the conductance-level dynamics numerically stable. Full configs for each entry are available under \texttt{drafts/dendritic-local-learning/configs/}.}
...
For tasks with latent pathway structure, the broadcast becomes a low-dimensional vector over router-inferred pathway roles. Local pathway activity gates this vector. Block-structured transport then carries it upward so earlier branches receive feedback aligned with their downstream descendants.
