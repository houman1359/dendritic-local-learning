...
\textbf{(C)}~Morphology-dependent shunting advantage on noise resilience.
...
With matched rank-$1$ broadcast, shunting is modestly better on MNIST, Fashion-MNIST, and context gating, including $0.803\pm0.006$ vs.\ $0.787\pm0.007$ for additive on context gating. The gap grows on inhibition-sensitive noise resilience once inhibitory conductance is present (Fig.~\ref{fig:competence_regime}B), and tree depth shifts the inhibitory operating point (Fig.~\ref{fig:competence_regime}C).

\section{Discussion and Limitations}
\label{sec:discussion}
The main result is a condition for faithful dendritic broadcast, not a claim that LocalCA universally replaces backpropagation. Rank-$1$ broadcast succeeds when the exact compartment-error field is sufficiently compressible, and shunting can help by changing input resistance along credit paths. The strongest causal evidence is that transported error closes much of the local-learning gap, while destroying learned inhibition collapses performance even when conductance scale is preserved. Boundaries remain: 5F is an empirical stabilizer, Eq.~\eqref{eq:compression_condition} is interpretive rather than a positive headline diagnostic, and routed/CIFAR-10 tasks expose where scalar broadcast needs higher-rank or transported signals.
