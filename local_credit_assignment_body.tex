...
\textbf{(A)}~Single dendritic E/I unit: nonnegative excitatory and inhibitory input streams drive a 3-level branched tree of conductance compartments; each branch carries E and I synapses; the gold-ringed soma matches the highlighted unit in B.
\textbf{(B)}~Network layer: E and I input pools feed $N$ dendritic E/I units; per-unit somas project to a task readout, and $\delta_0\!=\!\partial L/\partial V_0$ is the somatic error available for broadcast.
\textbf{(C)}~Credit assignment and broadcast taxonomy: exact credit varies by path through the conductance-stage path gains $\alpha_n\delta_0$, whereas rank-1 LocalCA shares one error field across compartments; the four broadcast modes used in the paper (Rank-1, Rank-$K$, Path, Oracle) trade bandwidth for fidelity. The local eligibility (orange) and broadcast factor (red) factorize the gradient as shown in the bottom equation strip.}
