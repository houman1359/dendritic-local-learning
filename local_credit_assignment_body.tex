...
\textbf{(A)}~A compartmental dendritic neuron with excitatory (blue circles) and inhibitory (red triangles) synapses arranged on a rooted tree. Voltages propagate to the soma through learned dendritic conductances (green arrows), which is the operating regime studied in the main feedforward experiments.
\textbf{(B)}~Shunting voltage integration. The branch voltage is set by a conductance equation in which inhibition enters the denominator; the paired histograms illustrate the visual intuition behind the paper, namely that shunting narrows the conductance-stage path-gain distribution while additive integration leaves it broad.
\textbf{(C)}~LocalCA rule hierarchy. 3F is the theorem-facing rule, 4F adds the heuristic morphology-aware factor $\rho$, and 5F adds the heuristic confidence-like factor $\phi$.
\textbf{(D)}~Broadcast modes. Scalar sends one shared field, per-soma sends an output-space vector when dimensions match and otherwise falls back to scalar, random low-rank adds a few unstructured channels, and structured pathway feedback preserves pathway identity when branches should receive different teaching signals.
\textbf{(E)}~Representative MNIST learning dynamics from the fixed-epoch Figure 1 sweep. Shunting with the stronger local wrappers tracks backprop much more closely than the additive control, giving a concrete preview before the paper turns to the sharper gradient-fidelity diagnostics in Fig.~\ref{fig:gradient_fidelity}.}
